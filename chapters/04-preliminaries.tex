...
A verbal predicate phrase is composed of a verbal phrase (VP) and optionally a complement as schematized in \textsc{ref}{ex:4.5} (see \sectref{sec:9.1} for more details).
...
The VP may be composed of a lexical verb and an auxiliary verb as in \textsc{ref}{ex:4.7}, which is called the auxiliary verb construction (\textsc{av}C) (see \sectref{sec:9.1.1}).
...
An adjectival predicate phrase is composed of an adjective and optionally a stative verb as schematized in \textsc{ref}{ex:4.9} (see \sectref{sec:9.2} for more details).
...
A nominal predicate phrase is composed of a nomina phrase (NP) and optionally a copula verb (\textsc{cop}) as schematized in \textsc{ref}{ex:4.12} (see \sectref{sec:9.2} for more details).
...
\z
  ...
  On the contrary, if a nominal predicate fulfills all of the conditions in \textsc{ref}{ex:4.15} and \REF{ex:4.18}, the copula verbs never appear as in (4-19 a-b).
...
A nominal may be derived from a verbal stem (see \sectref{sec:4.3.8.1}). A few nominals that have temporal meanings, e.g., \textit{kjuu} ‘today,’ \textit{acja} ‘tomorrow,’ and \textit{kinju} ‘yesterday,’ can be used adverbially (put another way, they can convert to adverbs with no formal change) as in \textsc{ref}{ex:4.48}.
...
\ea \label{ex:4.59} \ea  \textit{cja} ‘want’ [= \textsc{ref}{ex:4.36}] \label{ex:4.59a}\\\relax
...
We do not interpret \textit{{}-tu} (\textsc{advz}) as \textit{tu} (\textsc{com}) discussed in \sectref{sec:6.3.2.11} since the preceding form, e.g., /sijuzijuu/ in \textsc{ref}{ex:4.62} cannot take other case particles or cannot be followed by the copula verb. These facts mean that the form cannot be a nominal. Furthermore, this type of adverbialization cannot apply to adjectival stems that express a kind of emotion, e.g., *\textit{utumara+utumara-tu} (\textsc{red}+feel.strange-ADVZ).
