...
The modifier slot of an NP is not obligatory, and it can be filled by another NP (taking genitive case), adnominal word, and adnominal clause. Let us see some examples in the following sections.
...
If the NP modifier is address noun (see \sectref{sec:key:7.2}) such as \textit{anmaa} ‘mother’ or a nominal that contains \textit{-taa} (\textsc{pl}) (see \sectref{sec:key:6.4.1}), it does not take the genitive case, and only juxtaposition shows the possessive meaning as in (\ref{ex:6-4}a-b).
...
There are a few cases where the genitive case particle \textit{nu} can follow another case particle. The sequences of case particles are underlined below.
...
The head slot of an NP can be filled by the infinitive (see \sectref{sec:key:8.5.4}).
...
It should be noted that an NP can have recursive structure. A head nominal followed by the genitive particle can fill the modifier slot recursively as in \REF{ex:6-2}, whose construction is as follows: “[Modifier Head]\textsubscript{Modifier} Head.” In addition, a head modified by an adnominal clause can fill the head slot recursively, which is further modified by an adnominal as in (\ref{ex:4-46}b) in \sectref{sec:key:4.2.4}, whose construction is as follows: “Modifier [Modifier Head]\textsubscript{Head}.”

\subsection{Bound head (formal nouns)}

A head of an NP is usually a free form as in the previous section. However, some head nominals are bound forms; i.e. they cannot start an utterance by themselves and they always need some modifiers preceding them. Such nominals are called “formal nouns” in this grammar associated with the same term used in the traditional Japanese linguistics, although they used the term in a broader sense (cf. \citealt[664-665]{Martin1987[1975]}). The formal nouns in Yuwan can be modified by at least one of the following: adnominals, address nouns, or adnominal clauses.

So far, I have found thirteen formal nouns in my texts: \textit{kutu} ‘event’, \textit{hudu} ‘quantity’, \textit{bun} ‘share’, \textit{taməə} ‘sake’, \textit{hazɨ} ‘certainty’, \textit{nintəə} ‘people’, \textit{nagatɨɨ} ‘along’, \textit{hutəə/butəə/datəə} ‘vicinity’, \textit{turoo} ‘place’, \textit{mama} ‘still’, \textit{tui} ‘as,’ \textit{hui} ‘pretend,’ and \textit{sɨ} ‘thing; person; fact.’ Among them the formal noun \textit{sɨ} has a few characteristics different from others, which will be discussed in the final subsection (\sectref{sec:6.2.2.13}).
...
\ea\label{ex:6-22}
\ea With an adnominal\\{} \label{ex:6-22a}
...
In addition, \textit{nagatɨɨ} ‘along’ can be the head of a compound with verbal infinitives, and it means ‘while.’
...
The formal noun \textit{nagatɨɨ} in \REF{ex:6-23} is similar to the affixes following infinitives in \sectref{sec:8.4.4.3} since both of them are bound morphemes and follow verbal infinitives. However, they are different from each other since \textit{nagatɨɨ} can be modified by adnominals as in (\ref{ex:6-22a}), but the affixes in \sectref{sec:8.4.4.3} cannot.
...
\textit{nanziju=ccjɨ=nkja=ja}  \textit{sɨr-jur=\Highlight{sɨ}=ja}  \textit{waakja=bəi}   \textit{ar-an-tar=kai}\\
...
\textit{ura=ga}  \textit{jˀ-jur=\Highlight{sɨ}=nan}  \textit{hɨntoo=ja}  \textit{sɨr-jur-n} \textit{hɨntoo=ja}  \textit{sɨr-jur-sa}\\
...
\textit{ku-rɨ=ja}  \textit{mae=nu=\Highlight{sɨ}}  \textit{zjar=jaa}\\
...
\textit{wakar-an}  \textit{ka-n}  \textit{sɨr-tɨ}  \textit{juraw-tar=\Highlight{sɨ}=ja} \textit{sij-an}\\
...
\textit{nan=ga}  \textit{kunəəda}  \textit{umoor-tar=\Highlight{sɨ}=ja}  \textit{ku-n}   \textit{cˀju=nu}  \textit{k-təəra=i}\\
...
\glll uraasəə  ooja  iziran.jaa.\\
      \textit{\Highlight{ura-a=sɨ}=ja}  \textit{oo=ja}  \textit{izir-an=jaa}\\
...
\glll {\textbar}taken{\textbar}nusɨga  mutu  zja.\\
      \textit{\Highlight{taken=nu=sɨ}=ga}  \textit{mutu}  \textit{zjar}\\
      Taken=\textsc{gen}=\textsc{fn}=\textsc{nom}  original  \textsc{cop}\\
\glt ‘The things from Taken (place name) [i.e. pictures gathered in Taken] are originals (of the collection).’ [Co: 111113\_02.txt]

\ex Modified by an address noun\\
{\TM}
\glll  anmaasəə  dɨru?\\
\textit{\Highlight{anmaa=sɨ}=ja}  \textit{dɨ-ru}\\
mother=\textsc{fn}=\textsc{top}  which-\textsc{nlz}\\
\glt ‘Which one (is) mother’s?’ [El: 140227]
\z
\z

\noindent As mentioned before, the formal noun \textit{sɨ} can be attached to verbal stems directly as in (\ref{ex:6-10}a-b), which is similar to nominalizers (see \ref{sec:4.3.8.1}). However, \textit{sɨ} is totally different from those nominalizers in that it can be modified by the modifiers in (\ref{ex:6-12}a-c). On the other hand, the nominalizers cannot be modified by them. For example, the agentive nominalizer \textit{-jaa} ‘person’ cannot be modified by a genitive NP: */takennu jaa/ \textit{taken=nu-jaa} [Intended meaning] ‘A person from Taken.’

There is a characteristic unique to the formal noun \textit{sɨ}, which differentiates \textit{sɨ} from other formal nouns. \textit{sɨ} cannot be modified by an adnominal clause (with the exception of the case where \textit{-an} (\textsc{neg}) precedes \textit{sɨ}). Rather, it behaves like a verbal affix directly following a bound verbal stem (cf. affix-like clitics in \sectref{sec:key:4.2.2.2}). Relevant examples were already shown in (6-10 a-b, 6-11 a-b). Thus, I will compare \textit{sɨ} and another formal noun, e.g. \textit{turoo} ‘place,’ in (\ref{ex:6-13}a-b).
...
\textit{a-n}  \textit{nəɨsəə}  \textit{mut-tɨ}  \textit{\Highlight{ik-jur=sɨ}=ja}  \textit{nuu=n}   \textit{nə-an-ba}  \textit{jakkəə}\\
...
{\TM}
\glll ikjun  turookai?\\
\textit{\Highlight{ik-jur-n}}  \textit{\Highlight{turoo}=kai}\\
...
\textit{\Highlight{kam-an=sɨ}=ja}  \textit{jiccj-soo}  \textit{nə-an}\\
...
\textit{hanməə=ga}  \textit{\Highlight{kam-an}}  \textit{\Highlight{cˀju}}  \textit{nar-tɨ}  \textit{k-tɨ=joo}\\
...
If the allative case \textit{kaci} follows vowels, the following fusion frequently occurs. It is noteworthy that the fusion of //ci, si, zi// and \textit{kaci} does not make /Cəəci/ but makes /Cjəəci/.
...
The fusion of //i, ɨ, u// and \textit{kaci} (\textsc{all}) changes the original vowel positions, but the other short vowels retain their original positions. In the case of //ci, si, zi//, the fused forms are palatalized. I will show examples below.
...
A syllable must have a nucleus filled by a vowel (see \sectref{sec:key:2.3.1}). Thus, if the dative case \textit{n}, locative case \textit{nan} (\textsc{loc1}), or \textit{nən} (\textsc{loc1}) happens to make a syllable filled by a single consonant, an epenthetic vowel /i/ is inserted as a nucleus.


\ea\label{ex:6-35}
     \textit{n} (\textsc{dat}1) + //C\#// > /niC/
     
     \textit{nan} (\textsc{loc}1) + //C\#// > /na.niC/
     
     \textit{nən} (\textsc{loc}1) + //C\#// > /nə.niC/
\z

\ea\label{ex:6-36}
    \ea Dative\\ \label{ex:6-36a}
        \textit{jinga}  ‘man’  +  \textit{n}  (\textsc{dat}1)  +  \textit{n}  ‘also’ >  /jin.ga.nin/

    \ex Locative 1\\
        \ea  \textit{kun}  (\textsc{prox}.\textsc{adnz})  +  \textit{nan}  (\textsc{loc1})  +  \textit{n}  ‘also’ > /kun.na.nin/
            
        \ex  \textit{kun}  (\textsc{prox}.\textsc{adnz})  +  \textit{nən}  (\textsc{loc1})  +  \textit{n}  ‘also’ > /kun.nə.nin/
        \z
    \z
\z

\noindent In cases where \textit{n} (\textsc{dat}1) follows a syllable-final //n// (instead of preceding //n// such as (\ref{ex:6-36}a)), an epenthetic vowel /u/ is inserted between them by the application of a phonological rule disucussed in \sectref{sec:key:2.4.3}, e.g. \textit{bun} ‘the Bon festival’ + \textit{n} (\textsc{dat}1) > /bu.nun/. This raises the question of what happens in cases where \textit{n} (\textsc{dat}1) is surrounded by //n//s. In those cases, as mentioned before (at the beginning of \sectref{sec:key:2.4}), the morphophonemic rule \REF{ex:6-35} applies first, and that is sufficient in order to adjust the syllable structure.

\ea\label{ex:6-37}
    \textit{wan}  (1\textsc{sg})  +  \textit{n}  (\textsc{dat}1)  +  \textit{n}  ‘also’ > /wan.nin/ (*/wa.nu.nin/  )
\z

\subsubsection{Deletion in locative cases \textit{nan} (\textsc{loc1}) and \textit{nantɨ} (\textsc{loc2})}

It is possible that the locative cases \textit{nan} (\textsc{loc1}) and \textit{nantɨ} (\textsc{loc2}) become /n/ and /ntɨ/ respectively. In other words, //na// in their initial position can be deleted, when they follow a vowel.

\ea\label{ex:6-38}
     //V// + \textit{nan} (\textsc{loc}1) > /Vn/
     
     //V// + \textit{nantɨ} (\textsc{loc}2) > /Vntɨ/
\z

\ea\label{ex:6-39}
 \ea Locative 1      \\
    \textit{kuma}  ‘here’  +  \textit{nan}  (\textsc{loc1})  +  \textit{nu}  (\textsc{gen}) > /ku.man.nu/  \\

 \ex Locative 2      \\
    sja  ‘lower side’  +  nantɨ  (\textsc{loc2}) > /sjan.tɨ/        
 \z
\z

Additionally, if the locative case \textit{nan} (\textsc{loc1}) follows a vowel and also precedes a syllable filled by a single consonant (i.e. if both of the situations \REF{ex:6-35} and \REF{ex:6-38} occur at the same time), there are two possibilities. First, only the epenthesis of /i/ occurs as in \REF{ex:6-41a}. Second, both deletion of //na// and epenthesis of /i/ occur as in \REF{ex:6-41b}.

\ea\label{ex:6-40}
  //V// + \textit{nan} (\textsc{loc1}) + //C\#// > /VniC\#/
\z

\ea\label{ex:6-41} \textit{kuma}  ‘here’  +  \textit{nan}  (\textsc{loc1})  +  \textit{n}  ‘also’\\
    \ea\label{ex:6-41a}
    \glll  
    {} Input: kuma nan n \\
    > /i/-epenthesis: kuma nani n\\
    > Output: /ku.ma.na.nin/ \\

    \ex\label{ex:6-41b}
    \gllll  
    {} Input: kuma nan n \\
    > //na//-deletion: kuma n n\\
    > /i/-epenthesis: kuma ni n\\
    > Output: /ku.ma.nin/ \\
    \z
\z

In fact, when not followed by a syllable filled by a single consonant, the deletion of //na// rarely happens. That is, \textit{kuma} (\textsc{prox}.place) + \textit{nan} (\textsc{loc1}) > /kuma=nan/ is preferred. Actually, /kuma=n/ is judged as possible when I asked my consultants whether it can be used, but it is rarely uttered not only in the discourse, but also in elicitation. For this reason, the /ni/ is not regarded as the dative case \textit{n}, but is regarded as the deleted (and \textit{i}-inserted) form of \textit{nan} (\textsc{loc1}). Moreover, interpreting this /n/ as the deleted form of \textit{nan} (\textsc{loc1}) makes it easy to see the correspondence between \textit{nan} (\textsc{loc1}) and \textit{nantɨ} (\textsc{loc2}).
...
\item[d.] Lexical verb in the \textsc{avc} that expresses incapability or includes /nə-n/ (\textsc{rsl}-\textsc{neg});
...
[Context: Remembering a scene from the Pear Film]

{\TM}
\glll uziiga  mutɨ,  un  kˀwanu  muccjɨ  izjɨ,\\
      \textit{uzii=ga}  \textit{mur-tɨ}  \textit{u-n}  \textit{kˀwa=nu}     \textit{mut-tɨ}  \textit{ik-tɨ}\\
      old.man=\textsc{nom}  pick-\textsc{seq}  \textsc{mes}-\textsc{adnz}  child=\textsc{nom} have-\textsc{seq}  go-\textsc{seq}\\
\glt ‘The old man picked (the pears), and the child brought (them), and ...’ [\textsc{pf}: 090827\_02.txt]

\ex Subject of adjectival predicates\\
{\TM}
\glll  {\textbar}iro{\textbar}nu  kˀurusajaa.\\
\textit{iro=nu}  \textit{kˀuru-sa=jaa}\\
color=\textsc{nom}  black-\textsc{adj}=\textsc{sol}\\
\glt ‘(He) looks black.’ [Co: 120415\_00.txt]
...
In (\ref{ex:6-43}a), /anmataa/ \textit{anmaa-taa} (mother-\textsc{pl}) is the subject of the verbal predicate (whose head is the intransitive verb \textit{wur-} ‘exist’), and it takes the nominative case particle \textit{ga} (see \sectref{sec:6.4.1} about the non-associative use of the plural form \textit{anmaa-taa}). In (\ref{ex:6-43}b), \textit{uzii} ‘old man’ is also the subject of the verbal predicate (whose head is the transitive verb \textit{mur-} ‘pick’), and it takes the nominative case particle \textit{ga}. Similarly, \textit{u-n} \textit{kˀwa} (\textsc{mes}-\textsc{adnz} child) ‘that child’ is the subject of the verbal predicate (whose head is the transitive verb \textit{mut-} ‘have’), and it takes the nominative case particle \textit{nu}. In (\ref{ex:6-43}c), \textit{iro} ‘color’ is the subject of the adjectival predicate (whose head is \textit{kˀuru-sa} (black-\textsc{adj}) ‘black’), and it takes the nominative case particle \textit{nu}. In (\ref{ex:6-43}d), \textit{ku-ma} (\textsc{prox}-place) ‘here’ is the subject of the nominal predicate, and it takes the nominative case particle \textit{ga}. It should be noted that there are some situations where the nominative case does not appear. For example, the subject of an imperative sentence usually does not appear, but sometimes it can appear. In that case, the subject does not take the nominative case.
...
\textit{ura=ga}  \textit{tumai=nu}  \textit{ar-an}  \textit{tuki=n}\\
    2.\textsc{nhon}.\textsc{sg}=\textsc{nom}  night.shift=\textsc{nom}  \textsc{cop}-\textsc{neg}  time=\textsc{dat}1\\
    Subject  [NP  Copula verb]\textsubscript{Nomimal predicate phrase}  \\
\glt    ‘When you are not on the night shift, ...’ [Co: 111113\_02.txt]
\z

The above example shows that not only the subject, i.e. \textit{ura=ga} (2.\textsc{nhon}.\textsc{sg}=\textsc{nom}), but also the NP in the predicate, i.e. \textit{tumai=nu} (night.shift=\textsc{nom}), take the nominative case.

  With regard to (\ref{ex:6-42}d), the nominative case can be used to mark the lexical verbs in the auxiliary verb construction (\textsc{avc}) that express incapability or includes /nə-n/ \textit{nə-an} (\textsc{rsl}-\textsc{neg}).

\ea\label{ex:6-48}
 Lexical verbs in \textsc{avc} expressing incapability\\
...
\glt ‘If there is no food such as rice, (we) cannot keep body and soul together, so ...’ [Co: 120415\_01.txt]

  Lexical verbs in \textsc{avc} whose auxiliary verb is /nə-n/ \textit{nə-an} (\textsc{rsl}-\textsc{neg})
...
\glt ‘If (he) has not gone to eat the meal yet (and if he is not yet out) that, (he may be there). (But actually I) wonder if (he is).’ [Co:~110328\_00.txt]
...
burn-\textsc{seq}=\textsc{nom}  exist-\textsc{neg}-\textsc{csl}\\
...
pear=\textsc{acc}  one.\textsc{clf}  one.CLF  pick-\textsc{prog}-\textsc{ptcp}=\textsc{cfp}=\textsc{cfm}1\\
\glt ‘(The old man) is picking pears off one by one.’ [\textsc{pf}: 090222\_00.txt]
\z
\z

Both object NPs in (\ref{ex:6-52}a-b) take the accusative case particle \textit{ba}. Additionally, the accusative case \textit{ba} can be omitted as in \REF{ex:6-53}. It is noteworthy that the object \textit{nasi} ‘pear’ and the following verbal root \textit{mur-} ‘pick’ are not composing a compound in the phonological point of view. A compound has a single rising (or falling) pitch, but /nàsí mùtúǹ/ has more than two pitch changes.
...
\ea\label{ex:6-54}
 ...
 \ea
\glll amaba ikjoojəə.\\
\textit{a-ma=ba}  \textit{ik-oo=jəə}\\
\textsc{dist}-place=\textsc{acc}  go-\textsc{vol=cfm2}\\
\glt ‘(I) will go there [lit. to that place].’ [El: 120917.txt]
...
In the above example, \textit{n} (\textsc{dat}1) follows the infinitve of the \textit{wur-} ‘exist’, i.e. /wui/ \textit{wur-i} (exist-\textsc{inf}), and is followed by \textit{kara} (\textsc{abl}) meaning ‘from the time ...’. Such a phenomenon, i.e. the combination of an infinitive plus \textit{n} (DAT1) meaning the time of the event, is said to be common in Ryukyuan languages (Prof. Shigehisa Karimata, 2013 p.c.). There are no examples in my texts where \textit{n} (DAT1) is followed by \textit{kara} (ABL) if the preceding word is a nominal, e.g. *\textit{tuki=n=kara} (time=DAT1=ABL). Thus, it seems that the \textit{n} following a nominal would be different from \textit{n} following a verb. However, I will regard them as the same morpheme \textit{n} (DAT1) because of the following reasons: (a) both kinds of \textit{n} behave in the same way on morphophonological alternation; (b) \textit{n} (DAT1) following a nominal can also mean the time of the event.
...
In the above example, \textit{a-rɨ} (\textsc{dist}-\textsc{nlz}) ‘that person’ is the addressee of the verb \textit{jˀ-} ‘say’ and takes \textit{nkatɨ} (\textsc{dat}2). \textit{nkatɨ} (\textsc{dat2}) can co-occur with \textit{jˀ-} ‘say,’ \textit{hanas-} ‘talk,’ and \textit{jusɨr-} ‘teach.’ The origin of \textit{nkatɨ} (\textsc{dat2}) is not clear so far. Although we cannot determine the correct candidate for its origin, we can say a wrong candidate. The initial phoneme /n/ of \textit{nkatɨ} (\textsc{dat2}) is not made of the contraction of the genitive particle \textit{nu} (see \REF{ex:6-81} in \sectref{sec:key:6.3.2.14} for the contraction of the genitive \textit{nu}), because the demonstrative nominal does not take the genitive particle \textit{nu} if it indicates human (see \tabref{tab:key:44} in \sectref{sec:key:6.4} and \REF{ex:6-106} in \sectref{sec:6.4.2.1}). In \REF{ex:6-58}, the demonstrative /arɨ/ \textit{a-rɨ} (\textsc{dist}-\textsc{nlz}) clearly indicates a human referent, so it cannot take \textit{nu} (\textsc{gen}). That is, the /n/ of \textit{nkatɨ} (\textsc{dat2}) is not made of \textit{nu} (\textsc{gen}), at least considering the modern synchronic data.
...
\glt    ‘(The person\textit{\textsubscript{} }said) that I was making the person ill using (my) mouth, and ...’ [Co: 120415\_01.txt]
 ...
 \ex\relax[Context: Hearing that US spoke to the present author in the standard Japanese]\\
{\TM}
...
In (\ref{ex:6-68}a) \textit{kuci} ‘mouth’ is the instrument used to criticize someone, and it takes \textit{sjɨ} (\textsc{inst}). In (\ref{ex:6-68}b) \textit{hoogen} ‘dialect’ is the instrument used to express the speaker's message.

The next example is used to mean material, where the NP marked by \textit{sjɨ} (IN\textsc{st}) becomes a part of the result of action.
...
In (\ref{ex:6-69}), \textit{kusa} ‘grass’ is the material to cover the pot, and it takes \textit{sjɨ} (IN\textsc{st}).
...
\ea\label{ex:6-71}
 ...
 \ea\relax[Context: There are three boys who saw another boy bumping against a stone with bicycle, and the pears fell off the front basket; TM says, ‘The three (happened to) be passing by the place, and picked up the bicycle of the boy who bumped (there), and ...’]\\
{\TM}
...
\textit{ku-n}  \textit{cˀju=tu}  \textit{ku-n}  \textit{cˀju=tu=ga}    \textit{dɨkɨmun=joo}\\
[\textsc{prox}-\textsc{adnz}  person=\textsc{com=nom}  \textsc{prox}-\textsc{adnz}  person=\textsc{com}]     [genius]=\textsc{cfm}1\\
      {}[Subject]    {} {} {} [Nominal\text{ }predicate]\\
...
\ea\reax[Context: Remembering that the present author asked \textsc{tm} to pronounce ‘head’ and ‘knee’ in Yuwan]\\
{\TM}
...
In (\ref{ex:6-75(1)}a), each NP, i.e. /hamaicu+uzii/ ‘Hamaitsu’ and /waakjaa torataroouzii/ ‘my grandfather Torataroo’ being the subject of nominal predicate, is followed by \textit{tu} (\textsc{com}). Similarly, in (\ref{ex:6-75(1)}b), each NP, i.e. /kun cˀju/ ‘this person\textit{\textsubscript{i}}’ and /kun cˀju/ ‘this person\textit{\textsubscript{j}}’ being the subject of nominal predicate, is followed by \textit{tu} (\textsc{com}). In (\ref{ex:6-75(2)}a), each NP, i.e. \textit{cuburu} ‘head’ and \textit{cɨbusi} ‘knee’ being the object of transitive verb, is followed by \textit{tu} (\textsc{com}). Similarly, in (\ref{ex:6-75(2)}b), each NP, i.e. \textit{haori} ‘haori’ and \textit{kin} ‘cloth’ being the object of transitive verb, is followed by \textit{tu} (\textsc{com}).
...
\glt ‘If my classmates, i.e. Kikuko and her friends, were still alive, (if) they (were alive, the present author could study their dialects).’ [Co:~120415\_01.txt]
\z
\z
...
First, an NP followed by the nominative case fills the argument slot of a clause, and its head is the predicate phrase as in (\ref{ex:6-80(1)}a-b) (see \sectref{sec:key:4.1.1}). On the other hand, an NP followed by the genitive case fills the modifier slot of an NP, and its head is a nominal as in (\ref{ex:6-80(2)}a-b) (see \sectref{sec:key:6.1}).
...
\ea\label{ex:6-80(2)}
Filling the modifier slot of an NP\\
...
In the first two examples, both \textit{a-rɨ} (\textsc{dist}-\textsc{nlz}) ‘that person’ in (\ref{ex:6-80(1)}a) and \textit{kɨɨ} ‘tree’ in (\ref{ex:6-80(1)}b) fill the argument slots of the clauses. More specifically, they are subjects of the clauses. In the next two examples, however, the same NPs do not fill the arguments but fill the modifier slots of NPs. In (\ref{ex:6-80(2)}a), \textit{a-rɨ} (\textsc{dist}-\textsc{nlz}) ‘that person’ modifies the head nominal \textit{ututu} ‘younger sibling’ (about the contraction from \textit{a-rɨ=ga} > /agga/, see \sectref{sec:key:5.2.1}). In (\ref{ex:6-80(2)}b), \textit{kɨɨ} ‘tree’ modifies the head nominal \textit{sja} ‘(th place) under (something)’. It is true that each case particle in (6-80 a, c), i.e. /ga/, and those in (6-80 b, d), i.e. /nu/, have the same form respectively. However, I will propose that they should be regarded as different case particles.
...
In the following subsections, I will compare some case particles that have similar functions. In \sectref{sec:key:6.3.3.1}, dative 1, dative 2, and allative will be discussed. In \sectref{sec:key:6.3.3.2}, the locative 1, 2, and 3 will be discussed.

\subsubsection{Dative 1, dative 2, and allative}

All of the cases \textit{n} (\textsc{dat}1), \textit{nkatɨ} (\textsc{dat2}), and \textit{kaci} (\textsc{all}) may co-occur with verbs that have a meaning related with direction. The details of their differences are not very clear, but there are restrictions on their co-occurrence with their head verbs depending on the meanings of the verbs. The possibility of their co-occurrence with several verbs (or verbal affixes) is shown in the following table and examples. In \tabref{tab:key:42}, “+” means that the case particle can co-occur with the verbs (or verbal affixes), and “-” means cannot.
...
\ea\label{ex:6-82}
\ea Co-occurrence with \textit{-arɨr} (P\textsc{ass}) to mark the agent\\
{\TM}
...
\ex Co-occurrence with \textit{-as} (\textsc{caus}) to mark the causee\\
...
\ex Co-occurrence with \textit{kurɨr-} ‘give’ to mark the recipient\\
...
\ex Co-occurrence with \textit{jˀ-} ‘say’ to mark the recipient of the information\\
...
\glt    ‘Whom did you talk to?’ [El: 130820]

\ex Co-occurrence with \textit{nagɨr-} ‘throw’ to mark the goal\\
...
\ex Co-occurrence with \textit{ik-} ‘go’ to mark the goal\\
...
As far as the verbs (and the verbal affixes) in \tabref{tab:key:42} are concerned, we can say the following things. First, \textit{n} (\textsc{dat}1) can co-occur with several verbs or verbal affixes with the exception of \textit{nagɨr-} ‘throw’ and \textit{ik-} ‘go.’ Thus, \textit{n} (DAT1) seems not to be used to mark the goal in the narrow sense. In other words, the “goal” marked by \textit{n} (DAT1) is the recipient or causee. Secondly, \textit{kaci} (\textsc{all}) can co-occur with almost all of the verbs or verbal affixes with the exception of \textit{-arɨr} (P\textsc{ass}). In fact, \textit{-arɨr} (\textsc{pass}) has little meaning strongly related with direction. Thus, it may be possible to say that \textit{kaci} (ALL) can be used with verbs that have a meaning related with direction. Finally, \textit{nkatɨ} (\textsc{dat2}) can be used only with \textit{jˀ-} ‘say.’ As mentioned in \sectref{sec:key:6.3.2.4}, \textit{nkatɨ} (\textsc{dat2}) can be used only to mark the recipient of the information.

\subsubsection{Locative 1, locative 2, and locative 3}

All of the cases \textit{nan} (\textsc{loc1}), \textit{nantɨ} (\textsc{loc2}), and \textit{zjɨ} (\textsc{loc3}) can express the place where the action (or event) (indicated by the head verb) occurs. The details of their differences are not very clear, but there are restrictions on co-occurrence with verbs or the context where they are used. The possibility of co-occurrence with a few verbs and a nominal is shown in the following table and examples. In \tabref{tab:key:43}, “+” means that the case particle can co-occur with the verbs (or the nominals), and “-” means cannot.
...
\begin{tabularx}{\textwidth}{XXXXX}
\lsptoprule
Co-occurrence with  && Verbs &        Nominal\\
\midrule
...
\ea\label{ex:6-83}
 \ea Co-occurrence with \textit{wur-} ‘exist (animate)’\\
...
\ex Co-occurrence with \textit{ar-} ‘exist (inanimate)’\\
...
\ex Co-occurrence with \textit{udur-} ‘dance’\\
{\TM}
...
\ea\label{ex:6-84}
 Co-occurrence with \textit{ku-ma} ‘here’\\
...
\ea\label{ex:6-91}
  [Context: The speaker will not go anywhere, but drinks at the place where she is.]
...
Yuwan has several phenomena which are concerned with the animacy hierarchy in linguistic typology (more information about the animacy hierarchy, see \citealt{Silverstein1976}, \citealt{Comrie1989}, \citealt{Dixon1994}, \citealt{Whaley1997}, \citealt{Corbett2000}, and \citealt{Croft2003} among many others). For example, only personal pronouns have dual forms in Yuwan (see \sectref{sec:key:5.1}). Additionally, there are four other phenomena that are correlated with the animacy hierarchy: the choice of plural markers, the choice of tactics used in the modifier slot of an NP, the choice of the nominative case forms, and the choice of the existential verbs.

See the following table (\tabref{tab:key:44}), where “Intrg” is the abbreviation of human interrogatives, “Dmst” is the abbreviation of human demonstratives, and “Addr” is the abbreviation of address nouns mainly including elder kinship terms and personal names (see \sectref{sec:key:7.2} for more details). In addition, “Others” indicates the remained nominals including non-human demonstratives (i.e. the demonstrative nominals used to indicate non-human referents), and “Anm” is the abbreviation of animates and “Inanm” is the abbreviation of inanimates.

\begin{table}
\caption{\label{tab:key:44}Animacy hierarchy in Yuwan}
\begin{tabularx}{\textwidth}{XXXXXXXXX}
\lsptoprule
  & \multicolumn{3}{l}{Pronominals}  & Intrg  & Dmst  & Addr  & \multicolumn{2}{l}{Others}\\
  \midrule
  & 1st & 2nd & 3rd & & & & Anm & Inanm\\
  \midrule
  \multicolumn{9}{l}{Number markers}\\
  \text{ }SG \footnote{If a word ends with \textit{-ru} (\textsc{nlz}) or \textit{-rɨ} (\textsc{nlz}), it expresses the singularity, at least in natural discourse.} & \textit{-n} & \textit{-∅} & --- &  \textit{-ru} & \textit{-rɨ} & --- & --- & ---\\
  \text{ }DU & \textit{-ttəə} & \textit{-ttəə} & \textit{-ttəə} & --- & --- & --- & --- & ---\\
  \text{ }PL \footnote{This alignment depends on the text data. In the elicitation data, human demonstratives may take \textit{nkja} (\textsc{appr}), and non-human demonstratives may take \textit{-taa} (\textsc{pl}) (see §\ref{bkm:Ref361694047} for more details).} & \textit{-kja} & \textit{-kja} & --- & \textit{-taa} & \textit{-taa} & \textit{-taa} & \textit{nkja} & \textit{nkja}\\
  \midrule
  \multicolumn{9}{l}{NP modifiers}\\
  \text{ }SG & \textit{-a} & \textit{-a} & --- & \textit{-a} & \textit{ga} & \textsc{jxtp} & \textit{nu} & \textit{nu}\\
  \text{ }DU & \textit{ga} & \textit{ga} & \textit{ga} & --- & --- & --- & --- & ---\\
  \text{ }PL & \textit{-a} & \textit{-a} & --- & \textsc{\textsc{jxtp}} & \textsc{jxtp} & \textsc{jxtp} & \textit{nu} & \textit{nu}\\
  \midrule
  \multicolumn{9}{l}{Case particles}\\
  \text{ }S/A & \textit{ga} & \textit{ga} & \textit{ga} & ? \footnote{If the subject of a clause is an interrogative word, it does not take the nominative case particle, but takes the focus particle \textit{ga} (which is different from the nominative \textit{ga}). See §\ref{bkm:Ref367267321} and \sectref{sec:key:10.1} for more details.} & \textit{ga} & \textit{ga} & \textit{nu} & \textit{nu}\\
  \text{ }P & \textit{ba} & \textit{ba} & \textit{ba} & ? & \textit{ba} & \textit{ba} & \textit{ba} & \textit{ba/∅}\\
  \midrule
  \multicolumn{9}{l}{Existential verbs}\\
  & \textit{wur-} & \textit{wur-} & \textit{wur-} & \textit{wur-} & \textit{wur-} & \textit{wur-} & \textit{wur-} & \textit{ar-/nə-}\\
\lspbottomrule
\end{tabularx}
\end{table}

Generally, human interrogatives, e.g. \textit{ta-ru} (who-\textsc{nlz}) ‘who’ in Yuwan, does not come up for discussion of animacy hierarchy (at least in the research introduced above). The data of Yuwan shows that the distribution of human interrogatives is partly similar to personal pronominals with regard to the singular NP modifier, e.g. /ta-a/ (who-\textsc{adnz}) ‘whose’ and /ura-a/ (2.\textsc{nhon}.\textsc{sg}-\textsc{adnz}) ‘your.’ It is also partly similar to human demonstratives and address nouns with regard to the plural marker (and the plural form filling the NP modifier slot), e.g. /ta-t-taa/ (who-\textsc{nlz}-\textsc{pl}) ‘who (plural)’ and /a-t-taa/ (\textsc{dist}-\textsc{nlz}-\textsc{pl}) ‘those people.’ A possible reason why the human interrogative behaves in the same way with the personal pronominals is as follows. Human interrogatives and personal pronominals are literally “pronominal,” and also they obligatorily indicate human referents. On the other hand, the demonstrative nominals (and also the reflexive pronouns to be discussed in \sectref{sec:key:7.3}) may indicate non-human referents (see \sectref{sec:key:5.2}). Thus, the pronominal characteristic and the obligatoriness of indicating human referents may differentiate the personal pronominals and the human interrogatives from the others.
...
Yuwan has three plural markers: \textit{-kja}, \textit{-taa}, and \textit{nkja}. The choice of these markers depends on the types of preceding nominals (see the row of “Number markers” in Table \ref{tab:key:44}). The plural markers can be used to express both of the additive plural and the associative plural (see \citet[101-111]{Corbett2000} and \citet[12]{Daniel2005} about the terminologies).
Additive plurals are plurals of “x + x + x ...” type, where “x” indicates the nominal stem (ex: \textit{an warabɨ=nkja} (that child=\textsc{pl}) ‘those children’; i.e. “child + child + child”). Associative plurals are plurals of “x + y + z ...” type, where “x” indicates the nominal stem and “y” (or “z”) indicates the referent other than “x” (ex: \textit{waa maga=nkja} (my grandchild=\textsc{pl}) ‘my grandchild and his/her friend’).

However, the “plural” markers in Yuwan can be used to indicate a virtually single referent. I will present the relevant examples of \textit{-kja}, \textit{-taa}, and \textit{nkja} in turn below.
...
\ex\relax[Context: US told \textsc{tm} and \textsc{my} that TM knew everything, but TM said she knew nothing herself, but that her mother had known everything important.] = \REF{ex:5-8}\\
...
\ex \textit{-kja} (\textsc{pl}) can indicate a single referent as an example (to illustrate the proposition expressed by the clause where \textit{-kja} (\textsc{pl}) is included).\\
...
\glt ‘Yoneshige’s father and his [i.e. the present speaker’s] grandfather are cousins, (I heard).’ [Co: 110328\_00.txt]
...
\glt    ‘Minako\textit{\textsubscript{i}}, until she\textit{\textsubscript{i}} comes (here), ...’ [Co: 120415\_01.txt]
\z
...
The three plural markers -\textit{kja} (\textsc{pl}), \textit{-taa} (\textsc{pl}), and \textit{nkja} (\textsc{appr}) are chosen in this order corresponding to the lexical meaning of their preceding nominals, which is subject to the animacy hierarchy of Yuwan (see \tabref{tab:key:44}). A similar phenomenon, where several plural markers correspond to the animacy hierarchy, is found in other Ryukyuan languages, e.g. Ogami (Southern Ryukyuan) \citep[133]{Pellard2010}, and also in other languages, e.g. Eastern Huasteca Nahuatl \citep[77-78]{Corbett2000}. The verb in Yuwan does not show any number agreement with the arguments.

First, personal pronominals use \textit{-kja} (\textsc{pl}) to express the plural (or approximative) meaning (see \sectref{sec:key:5.1}). In (\ref{ex:6-99}a), the first person pronoun has its plural form \textit{waa-kja} (1-\textsc{pl}). In (\ref{ex:6-99}b), the second person honorific pronoun has its plural form \textit{naa-kja} (2.\textsc{hon}-\textsc{pl}). In (\ref{ex:6-99}c), the second person non-honorific pronoun has its plural form as \textit{ura-kja} (2.\textsc{nhon}-\textsc{pl}).
...
\z
  ...
  Finally, the other nominals use \textit{nkja} (\textsc{appr}) to express the plural (or approximative) meaning. If indefinite pronouns or demonstrative pronouns do not indicate human referents, they express the plurality using \textit{nkja} (\textsc{appr}) as in (\ref{ex:6-101}a-b). On the other hand, the reflexive pronoun \textit{nusi} (\textsc{rfl}) is also used with \textit{nkja} (\textsc{appr}) to indicate the plurarity, although the referent is a human, i.e. the hearer, as in (\ref{ex:6-101}c). Common nouns are always used with \textit{nkja} (\textsc{appr}) despite the referents being humans or non-humans as in (\ref{ex:6-101}d-e).
...
\z
  ...
  \textit{nkja} (\textsc{appr}) can follow other plural markers, i.e. \textit{-kja=nkja} (\textsc{pl}=\textsc{appr}) and \textit{-taa=nkja} (\textsc{pl}=APPR). In those cases, \textit{nkja} (\textsc{appr}) ignores the correspondence with the animacy hierarchy. First, let us see examples of \textit{-kja}=\textit{nkja} (\textsc{pl}=APPR).

\ea\label{ex:6-102}
  Double plural marking

 \ea Personal pronominal (1\textsuperscript{st} person)\\{}
[Context: Looking at a picture, where there were a few men]
...
Finally, there is also an example of double marking of \textit{nkja} (\textsc{appr}). However, it seems unproductive, since there is only one that example in my texts.

\ea\label{ex:6-104}
  Double plural marking\\{}
  Common noun\\{}
  [Context: Remembering the old days when Amami Ōshima was occupied by the US military]
...
\z
  ...
  Fourth, most of the other nominals in the singular take the genitive case particle \textit{nu} when they fill the modifier slot of an NP. In (\ref{ex:6-108}a), the non-human interrogative \textit{nuu} ‘what’ takes the genitive particle \textit{nu}. In (\ref{ex:6-108}b), the non-human demonstrative \textit{a-rɨ} ‘that’ takes the genitive particle \textit{nu}. In (\ref{ex:6-108}c), both common nouns \textit{zii} ‘ground’ and \textit{micja} ‘soil’ take the genitive particle \textit{nu}.
...
The above demonstratives do not indicate humans, but they can take \textit{ga} (\textsc{gen}). The flexible correspondence with the animacy hierarchy found in the above examples was not found in the behavior of plural markers in the text corpus, where human demonstratives always take \textit{-taa} (\textsc{pl}), and non-human demonstratives do not take it (see \sectref{sec:key:5.2.1} about the data from elicitation).
  ...
  The behaviour of words in the singular to fill the modifier slot of an NP was shown above; then, we will see the one in the plural in the following section.
...
Human interrogatives and human demonstratives in the plural use juxtaposition to fill the modifier slot of an NP, which is totally different from the means they use in the singular forms (see \sectref{sec:key:6.4.1.2}). Considering that juxtaposition is used in the lower (or right side) rank of the animacy hierarchy in the singular (i.e. address nouns), the pluralization seems to cause decreasing of the animacy in Yuwan (see also the remark on the double plural marking in the end of \sectref{sec:key:6.4.1.2}).

  Third, the other nominals in the plural take the genitive case particle \textit{nu} when they fill the modifier slot of an NP. So far, there is no use of non-human plural interrogatives in the modifier slot of an NP. In (\ref{ex:6-113}a), the non-human demonstrative in the plural \textit{a-rɨ=nkja} (\textsc{dist}-\textsc{nlz}=\textsc{appr}) takes the genitive particle \textit{nu}. In (\ref{ex:6-113}b), the common noun in the plural \textit{dusi=nkja} (friend=\textsc{appr}) also takes the genitive particle \textit{nu}.
...
Common nouns and inanimate nouns (i.e. “Others” in \tabref{tab:key:44}) hardly appear as NP modifiers in the plural. I have not found any example where a non-human demonstrative in the plural takes \textit{ga} (\textsc{gen}), which is clearly different from the case of non-human demonstratives in the singular discussed in \REF{ex:6-110} in \sectref{sec:6.4.2.1}.
...
Only the personal pronouns have the dual forms, i.e. \textit{wattəə} (1\textsc{du}) ‘the two of us,’ \textit{nattəə} (2.\textsc{hon}.DU) ‘the two of you (honorific), \textit{urattəə} (2.\textsc{nhon}.DU) ‘the two of you (non-honorific),’ and \textit{nattəə} (3DU) ‘the two of them’ (see \sectref{sec:key:5.1}). These dual forms take \textit{ga} (\textsc{gen}) when they fill the modifier slot of an NP as in (\ref{ex:6-115}a-d).
...
In (\ref{ex:6-118}a-d), the nominals in the lowest (or rightmost) rank of the animacy hierarchy take \textit{nu} (\textsc{nom}).
  ...
  In the end of \sectref{sec:key:6.4.1.2}, it was mentioned that there can be a sequence of plural markers, i.e. \textit{-taa=nkja} (\textsc{pl}=\textsc{appr}), where the choice of nominative particle does not change as in (\ref{ex:6-74}b) or (\ref{ex:6-103}c).

\subsubsection{\textit{ga} (\textsc{nom}) prevails obligatorily if the clause has a nominal predicate}

As we have seen in the end of the previous section, usually the nominals in the lowest (or rightmost) rank of the animacy hierarchy take \textit{nu} (\textsc{nom}). There are, however, several cases where this is not the case. First of all, I will present the case where the predicate is filled by NPs, i.e. nominal predicates. In that case, the subject NP always takes \textit{ga} (not \textit{nu}).
...
The subjects of nominal predicates, i.e. \textit{a-rɨ} ‘that’ in (\ref{ex:6-119}a), \textit{ku-rɨ} ‘this’ in (\ref{ex:6-119}b), \textit{cuburu} ‘head’ in (\ref{ex:6-119}c), and \textit{jaa} ‘house’ in (\ref{ex:6-119}d), take \textit{ga} (\textsc{nom}), in spite of their being non-human demonstratives or common nouns.
...
These examples show that the subjects of the nominal predicates filled by the infinitive also take \textit{ga} (\textsc{nom}) in spite of their being common nouns, i.e. \textit{kusabutuu} ‘grass’ in (\ref{ex:6-120}a) or \textit{mizjuu} ‘ditch’ in (\ref{ex:6-120}b).
...
In (\ref{ex:6-122}a-b), \textit{mɨɨ} ‘eye’ is not the subject of the clauses, since the acceptability of the use of the auxiliary honorific verb is determined by its preceding NPs, i.e. \textit{a-n} \textit{sinsjei} ‘that teacher’ in (122 a) or \textit{a-n} \textit{warabɨ} ‘that child’ in (\ref{ex:6-122}b), both of which are the subjects of the above sentences (see \chapref{sec:3}).
...
In (\ref{ex:6-123}a-b), \textit{mɨɨ} ‘eye’ is a common noun, but takes \textit{ga} (\textsc{nom}) and the clauses as a whole mean the incapability of the experiencer. In (\ref{ex:6-123}c), \textit{mimi} ‘ear’ is also a common noun, but takes \textit{ga} (\textsc{nom}) and the clause as a whole means the incapability of the experiencer. In (\ref{ex:6-123}d), \textit{hanməə} ‘meal’ is a common noun, but takes \textit{ga} (\textsc{nom}) and the clause as a whole means the incapability of the experiencer. Finally in (\ref{ex:6-123}e), \textit{wadai} ‘topic’ is a common noun, but takes \textit{ga} (\textsc{nom}) and the clause as a whole means the incapability of the experiencer. The verbal roots in (\ref{ex:6-123} a-e), i.e. \textit{mj-} ‘see,’ \textit{kik-} ‘hear,’ \textit{kam-} ‘eat,’ and \textit{sɨr-} ‘do’ express capability, even though they do not include any morpheme that especially means capability.
...
\textit{mɨɨ=ga}  \textit{mj-ar-an-ba}  \textit{naa}  \textit{ta-ru=ccjɨ=ja}  \textit{wakar-an}\\
...
\ea\label{ex:6-125}
 \ea Lexical verb in \textsc{avc} expressing incapability [= (\ref{ex:6-48}a)]\\

 {\TM}
\gllll kumɨnkjanu  nənboo,  kadɨga  ikjankara,\\
\textit{kumɨ=nkja=nu}  \textit{nə-an-boo}  \textit{kam-tɨ=ga}  \textit{ik-an=kara}\\
    rice=\textsc{appr}=\textsc{nom}  exist-\textsc{neg}-\textsc{cnd}  eat-\textsc{seq}=\textsc{nom}  go-\textsc{neg}=\textsc{csl}\\
        {} {} Lex.~verb  Aux.~verb\\
\glt    ‘If there is no food such as rice, (we) cannot keep body and soul together, so ...’ [Co: 120415\_01.txt]
...
These verbs are not “core arguments” since they are not nominals. However, the environements where \textit{ga} (\textsc{nom}) appears in (\ref{ex:6-125}a-b) are very similar to those of nominals as in \REF{ex:6-123}. One may think that the \textit{ga} (\textsc{nom}) in this section is the focus particle \textit{ga} in \sectref{sec:key:10.1.2.2.} In fact, I cannot rule this possibility out (see \sectref{sec:key:6.4.3.7}).
...
In (\ref{ex:6-126}a-b), \textit{kui} ‘voice’ is not the subject of the clauses, since the acceptability of the use of the auxiliary honorific verb \textit{moor-} is determined by its preceding NPs, i.e. \textit{naakjaa} \textit{anmaa-taa} ‘your mother’ or \textit{an} \textit{warabɨ} ‘that child,’ which are the subjects of the above sentences (see \chapref{sec:3}). If a clause has an adjectival predicate, the core arguments tends to choose \textit{ga} (\textsc{nom}) rather than \textit{nu} (\textsc{nom}) as in (\ref{ex:6-127}a-d). However, the adjectival predicate in the honorific \textsc{avc} does not induce such preference, and the core argument takes \textit{nu} (not \textit{ga}) as in (\ref{ex:6-126}a), at least in elicitation.
...
The core arguments, i.e. \textit{ku-rɨ} ‘this [i.e. stature]’ as in (\ref{ex:6-127}a), \textit{a-rɨ} ‘that (butterfly)’ as in (\ref{ex:6-127}b), \textit{kjoodəə=nkja} ‘brothers (and relatives)’ as in (\ref{ex:6-127}c), and \textit{jaa} ‘house’ as in (\ref{ex:6-127}d), take \textit{ga} (\textsc{nom}) in spite of their being non-human demonstratives or common nouns. I have not yet found any example in my text data where the non-human demonstrative takes \textit{nu} (\textsc{nom}) with adjectival predicates.
...
In (\ref{ex:6-129}a-b), \textit{kanɨ} ‘money’ is not the subject of the clauses, since the acceptability of the use of the auxiliary honorific verb \textit{moor-} is determined by its preceding NPs, i.e. \textit{a-n} \textit{sinsjei} ‘that teacher’ or \textit{a-n} \textit{warabɨ} ‘that child,’ which are the subjects of the above sentences (see \chapref{sec:3}).
...
If the subject NP indicates a referent that is both definite and human, its nominative case sometimes becomes \textit{ga} (\textsc{nom}).
...
\ex\relax[Context: The Motoyama family rented a shop that had been closed.]\\
{\TM}
...
hey  \textsc{mes}-place=\textsc{loc1}  Motoyama=\textsc{nom}  rent {} close-\textsc{rsl}-\textsc{pst}-\textsc{csl}   rentf-\textsc{seq}  \textsc{mes}-\textsc{adnz}=\textsc{loc1}  what-\textsc{indf}\\
...
\ea\label{ex:6-136}
  [Context: The very beginning of the monologue. {\TM} ‘(I will) start from the scene (where a man) picks pears off. There is a pear tree, (i.e.) a big tree, ...’]

{\TM}
\glll unnəntɨ  uziiga  cˀjui  joonasi    mutunwake.\\
\textit{u-n=nəntɨ}  \textit{uzii=ga}  \textit{cˀjui}  \textit{joonasi} \textit{mur-tur-n=wake}\\
    \textsc{mes}-\textsc{adnz}=\textsc{loc2}  old.man=\textsc{nom}  one.\textsc{clf}.person  pear   pick-\textsc{prog}-\textsc{ptcp}=\textsc{cfp}\\
\glt    ‘There, an old man is picking pears off.’ [\textsc{pf}: 090225\_00.txt]
\z
...
The alignment of the plural markers and NP modifiers in the animacy hierarchy is less flexible than that of the nominative case. The plural markers express the plurality of NPs. The NP modifiers express the relation within the NPs. Thus, both the plural markers and NP modifiers are parameters whose value is determined only within the NP. However, the nominative case is different from them, since it is linked with the relation between the NP and the predicate. It is noteworthy that the characteristics discussed in \sectref{sec:key:6.4.3.2} - \sectref{sec:key:6.4.3.5} are all characterized by low transitivity. Both the nominal predicate (in \sectref{sec:key:6.4.3.2}) and the adjectival predicate (in \sectref{sec:key:6.4.3.4}) have less (prototypical) transitivity, because they do not cause any change on any target (cf. \citealt{Tsunoda1991}: 72). Additionally, the negative pole, i.e. incapability as in \sectref{sec:key:6.4.3.3} and non-existence as in \sectref{sec:key:6.4.3.5}, is thought to have less transitivity (\citealt{HopperThompson1980}: 252).

However, it should be noted that all of the prior use of \textit{ga} (\textsc{nom}) in \sectref{sec:key:6.4.3.2} - 6.4.3.6 may be regarded as the use of the focus particle \textit{ga} (\textsc{foc}) (see \sectref{sec:key:10.1.2.2}). As mentioned in \sectref{sec:key:6.4.3.3}, I could not completely rule this possibility out. We need to clarify the details of this problem in future research.

Comparing with plural markers and NP modifiers, the nominative case is very sensitive to the definiteness of the NP. The example \REF{ex:6-109} in \sectref{sec:6.4.2.1} showed that NP modifiers are not sensitive to the definiteness of the whole NP, but that they are sensitive to the definiteness of the head nominal of the NP. Similarly, the plural markers are not sensitive to the definiteness of the whole NP, which is shown below.
