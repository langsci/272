...
The verbal morphology of Yuwan is agglutinative; it begins with a root, which is followed by an affix (or affixes) (see \sectref{sec:8.1}). There is no number (or gender) agreement between arguments and verbs in Yuwan. Inflectional morphology of Yuwan is not straightforward; a certain group of inflectional affixes cannot directly follow the verbal root, but always take a group of derivational affixes (see \sectref{sec:8.1}). The verbal morphology of Yuwan is rich in morphophonological alternation (see \sectref{sec:8.2}). The clausal types, i.e. main clause, adnominal (or relative) clause, nominal clause, and adverbial clause, can be expressed by the word-final inflectional affix. For example, a clause ending with \textit{-ɨ} (\textsc{imp}) is a main clause, but a clause ending with \textit{-n} (\textsc{ptcp}) (and without any focus on another constituent in the same clause) is an adnominal clause (see \sectref{sec:8.4}). Regarding tense, aspect, and modality, each of them can be expressed by verbal affixes, although they can be expressed by other morphosyntactic means. Tense affixes have the opposition of non-past vs past. Aspectual affixes express progressive, resultative, non-progressive, or habitual (see \sectref{sec:8.5.1.4} - \sectref{sec:8.5.1.6}). Modality is grammaticalized as a restricted set of mood affixes, e.g. the inferential affix \textit{-oo}. However, it typically surfaces in the tense affixes; the tense marker \textit{-tar} (\textsc{pst}) (in the finite-form use) expresses the speaker’s confidence in the factuality of the event (see \sectref{sec:8.4.1.1}).

\section{The structures of the verb}
\label{bkm:Ref303739828}

The verb has the structures as in (\ref{ex:8-1}a-b), which begin with a root and ends with an inflectional affix. Roughly speaking, the initial root and the final inflectional affix are obligatory, and the medial affixes are all optional; more details are explained later.

\ea\label{ex:8-1}
  Structures of the verb

\ea

\gll  Root \textit{-as} \textit{-arɨr} \textit{-tuk} \textit{-arɨr} \textit{-tur} \textit{-jawur} \textit{-an} \textit{-təər} \textit{-tar} -Inflection\\
    {} \textsc{ caus} \textsc{ pass} \textsc{ prpr} \textsc{ \Highlight{cap}} \textsc{ \Highlight{prog}} \textsc{ \Highlight{pol}} \textsc{ \Highlight{neg}} \textsc{ rsl} \textsc{ pst}\\

\ex
\gll  Root \textit{-as} \textit{-arɨr} \textit{-tuk} \textit{-jur} \textit{-təər} \textit{-tar} -Inflection\\
    {} \textsc{ caus} \textsc{ pass} \textsc{ prpr} \textsc{ \Highlight{act}} \textsc{ rsl} \textsc{ pst}\\
\z
\z

\noindent
The structures in (\ref{ex:8-1}a-b) can be paraphrased that the affix \textit{-jur} (\textsc{act}) cannot co-occur with \textit{-arɨr} (\textsc{cap}), \textit{-tur} (\textsc{prog}), \textit{-jawur} (\textsc{pol}), nor \textit{-an} (\textsc{neg}); all of these affixes are underlined in (\ref{ex:8-1}a-b). Other impossible combinations are summarized below, where “impossible combinations” means that the combinations have not appeared in my texts, or that the present author cannot find proper contexts for the questions in elicitation.

\ea\label{ex:8-2}
  Impossible combinations
    \ea *\textit{-arɨr} (\textsc{pass})   +  \textit{-arɨr} (\textsc{cap})
    \ex *\textit{-arɨr} (\textsc{pass})   +  \textit{-jur} (\textsc{act})
...
The possibility of combinations described above is induced by the combinations of two affixes found in the text corpus and my data from the interview. Three or more combinations of derivational affixes are not so common in the text corpus, and to find proper contexts to investigate such a combination is so difficult that their possibility is not clear so far.

  In the top of this section, I said the word-final inflectional affix in a verb is obligatory but that the preceding affixes are optional; however, the morphology of Yuwan is a little more complicated. The word-final inflectional affixes in Yuwan can be categorized into two distinct groups, one of which cannot directly follow the verbal root, \textit{-as} (\textsc{caus}), and \textit{-tuk} (\textsc{prpr}), and obligatorily needs a certain affix in (\ref{ex:8-5}) to precede; those affixes are called “indirect inflections” in this grammar. The other affixes, which can directly follow the verbal root, \textit{-as} (\textsc{caus}), and \textit{-tuk} (\textsc{prpr}) are called “direct inflections” in this grammar. The subcategories of inflectional affixes, i.e. finite-form affixes, participial affix, converbal affix, and infinitival affix, will be explained in \sectref{sec:8.4}.

\ea\label{ex:8-3}
  Inflectional affixes
\ea Direct inflections: Can directly follow the verbal root
    %[blank space and line feed are used on behalf of hangindent]
    
    i. \text{ }\text{ }Finite-form affixes  :
    
    \text{ }\text{ } \textit{-oo} (\textsc{int}), \textit{-ɨ} (\textsc{imp}), \textit{-na} (\textsc{proh}), \textit{-ɨba} (\textsc{sugs}), \textit{-azɨi} (\textsc{neg}.\textsc{plq}), \textit{-tar} (\textsc{pst})

    ii. \text{ }Participial affix  :\textit{-an} (\textsc{neg})

    iii. Converbal affixes  :
    
    \text{ }\text{ } \textit{-ba} (\textsc{csl}), \textit{-boo} (\textsc{cnd}), \textit{-tɨ} (\textsc{seq}), \textit{-təəra} ‘after’, \textit{-tai} (\textsc{lst}), \textit{-jagacinaa}
    
    \text{ }\text{ } (\textsc{sim}), \textit{-gadɨ} ‘until’

    iv. Infinitival affix  :\textit{-i}/\textit{-∅} (\textsc{inf})

\ex Indirect inflections: Cannot directly follow the verbal root

    i. \text{ }\text{ }Finite-form affixes  :
    
    \text{ }\text{ } \textit{-i} (\textsc{npst}), \textit{-oo} (\textsc{infr}), \textit{-mɨ} (\textsc{plq}), \textit{-sa} (\textsc{pol}), \textit{-sɨga} (POL), \textit{-u} (\textsc{pfc})

    ii. \text{ }Participial affix  : \textit{-n} (\textsc{ptcp})

    iii. Converbal affixes  : \textit{-tu} (\textsc{csl}), \textit{-too} (\textsc{csl}), \textit{-nən} (\textsc{seq})
\z
\z

On the one hand, direct inflections can directly follow the verbal root. On the other hand, indirect inflections cannot, but need another affix to precede. The minimal combinations with the above two types of inflectional affixes are shown below.

\ea\label{ex:8-4}
  Minimal combinations
\ea Root  +  Direct inflection        
    
    e.g.  /turoo/  \textit{tur-oo}  (take-\textsc{int})  ‘will take’


\ex Root  + Derivational affix + Indirect inflection    
    
    e.g.  /tujui/  \textit{tur-jur-i}  (take-\textsc{act}-\textsc{npst})  ‘take’
\z
\z

The non-past affixe \textit{-i} is an indirect inflection and cannot follow the verbal root directly: */tui/ \textit{tur-i} (take-\textsc{npst}) is not permitted. The affixes required by indirect inflections are called “connector derivations” in this grammar. I will show the list below.

\ea\label{ex:8-5}
  Connector derivations

\gll 
    \textit{-arɨr}, \textit{-arɨr}, \textit{-tur}, \textit{-jawur}, \textit{-an}, \textit{-təər}, \textit{-tar}, or \textit{-jur}\\
    \textsc{ pass} \textsc{ cap} \textsc{ prog} \textsc{ pol} \textsc{ neg} \textsc{ rsl} \textsc{ pst} {} \textsc{ act}\\

\z

\noindent
The above arrangement shows that if the word-final affix belongs to the indirect inflections in (\ref{ex:8-3}b), one of the derivational affixes in \REF{ex:8-5} must precede them. The other derivational affixes, i.e. \textit{-as} (\textsc{caus}) and \textit{-tuk} (\textsc{prpr}), cannot directly precede indirect inflections, and they are called “non-connector derivations” in this grammar. It is noteworthy that three kinds of verbal roots, i.e. the existential verbal root, the copula verbal root, and the stative verbal root, can take indirect inflections directly (see \sectref{sec:8.3.5}). There are some restrictions on the combinations between these affixes and indirect inflections. For example, there is no combination made of \textit{-an} (\textsc{neg}) plus \textit{-i} (\textsc{npst}). The possible combinations between derivational affixes and inflectional affixes will be shown in \sectref{sec:8.4}.

There are two special affixes: \textit{-an} (\textsc{neg}) and \textit{-tar} (\textsc{pst}). In the previous verbal structures in \REF{ex:8-1}, they are not in word-final positions. They can, however, stand in a word-final position without any inflectional affix. For example, /turan/ \textit{tur-an} (take-\textsc{neg}) ‘don’t take,’ and /tuta/ \textit{tur-tar} (take-\textsc{pst}) ‘took.’ In other words, \textit{-an} (\textsc{neg}) and \textit{-tar} (\textsc{pst}) can behave similarly with the inflectional affixes, which is shown in \REF{ex:8-6}.

\ea\label{ex:8-6}
\ea Structure ending with \textit{-an} (\textsc{neg})


    \gll
    Root  \textit{-as}  \texit{-arɨr} \textit{-tuk}  \textit{-arɨr}  \textit{-tur}  \textit{-jawur} \textit{-an}\\
    {} \textsc{ caus}  \textsc{ pass}  \textsc{ prpr}  \textsc{ cap}  \textsc{ prog}  \textsc{ pol}  \textsc{ neg}\\

\ex Structures ending with \textit{-tar} (\textsc{pst})

    \gll
    i. \text{ }Root  \textit{-as}  \texit{-arɨr} \textit{-tuk}  \textit{-arɨr}  \textit{-tur}  \textit{-jawur} \textit{-an}  \textit{-təər} \textit{-tar}\\
    {} {} \textsc{ caus}  \textsc{ pass}  \textsc{ prpr}  \textsc{ cap}  \textsc{ prog}  \textsc{ pol}  \textsc{ neg}  \textsc{ rsl}  \textsc{ pst}\\

    \gll
    ii. Root  \textit{-as}  \texit{-arɨr} \textit{-tuk} \textit{-jur} \textit{-təər} \textit{-tar}\\
    {} {} \textsc{ caus}  \textsc{ pass}  \textsc{ prpr} \textsc{ act} \textsc{ rsl}  \textsc{ pst}\\
    
\z
\z

\noindent
The affixes \textit{-an} (\textsc{neg}) and \textit{-tar} (\textsc{pst}) in word-final positions can be analyzed as “direct inflections” since they can directly follow verbal roots. It should be noted that these affixes “can” finish a verb. Therefore, they are free to finish the verbal string, and can continue it. For example, \textit{-an} (\textsc{neg}) can be followed by \textit{-ba} (\textsc{csl}), or \textit{-tar} (\textsc{pst}) can be followed by \textit{-oo} (\textsc{infr}): /turanba/ \textit{tur-an-ba} (take-\textsc{neg}-\textsc{csl}) ‘because (someone) does not take’ and /tutaroo/ \textit{tur-tar-oo} (take-\textsc{pst}-infr) ‘may have taken.’ In fact, the above analysis in \REF{ex:8-6} suggests that there are no zero inflectional affixes that follow \textit{-an} (\textsc{neg}) or \textit{-tar} (\textsc{pst}). In other words, we do not accept the analysis that presupposes a zero inflectional affix as in \REF{ex:8-7}, where the dotted lines (“...”) mean that there are several more candidates of derivational/inflectional affixes.

\ea\label{ex:8-7}
  Analysis not to be accepted

\glll
Root  [\textit{-as  -arɨr ... -an  -təər}] [\textit{-tar}/\textit{-i} \texit{-∅}/\textit{-oo}/...]\\
{} [\textsc{ caus  pass  ...  neg  rsl}] [\textsc{ pst}/\textsc{npst} \textsc{ ass}/\textsc{inf}/...]\\
{} [Derivations] [Inflections]\\
\z

The above analysis shows that the tense contrast is expressed in the penultimate slot of the verb: \textit{-tar} (\textsc{pst}) vs. \textit{-i}  (\textsc{npst}). Additionally, a new zero affix is postulated in the final slot of the verb, i.e. \textit{-∅} (\textsc{ass}), whose grammatical function is assertion. In this analysis, the final and penultimate slots would be inflectional. We do not take this zero-affix analysis, because the analysis postulates the zero affix \textit{-∅} (\textsc{ass}), which does not have any non-zero allomorph. This kind of zero morpheme is less convincing than another zero morpheme that has a non-zero allomorph (cf. \citealt{Haas1974}: 49). I will show examples in (\ref{ex:8-8}a-b).

\ea\label{ex:8-8}

\ea
\glll    wuduija sɨranta.\\
    \textit{wudur-i=ja}  \textit{sɨr-an-tar-∅}\\
    dance-\textsc{inf=top} do\textsc{-neg-pst-ass}\\
\glt ‘(I) didn't (have) a dance.’ [El: 111107]
...
\glt  ‘Probably (a person like) you would not carry the miscanthus.’

[Co: 110328\_00.txt]
\z
\z

See the contrasts of verbal forms between (\ref{ex:8-8}a) and (\ref{ex:8-8}b). The assertive affix \textit{-∅} (\textsc{ass}) is paradigmatically contrasted with another inflectional affix, i.e. \textit{-oo} (\textsc{inf}). The assertive affix, however, has no variant form other than \textit{-∅}. Therefore, we do not take the zero-morpheme analysis, and admit special kinds of affixes that can both close and continue the verbal stems. I will call those affixes “bilateral inflections” in this grammar.
The word-final use of \textit{-tar} (\textsc{pst}) will be discussed in \sectref{sec:8.4.1.1}. The word-final use of \textit{-an} (\textsc{neg}) will be discussed in \sectref{sec:8.4.2.2}. The non-word-final use of these affixes will be discussed in \sectref{sec:8.5.1.9}. The bilateral affixes \textit{-tar} (\textsc{pst}) and \textit{-an} (\textsc{neg}) can be included into connector derivations in a broad sense as in \REF{ex:8-5}, but excluded in a narrow sense as in \tabref{tab:55}.

All of the above verbal affixes can be summarized as in \tabref{tab:55} using the morphosyntactic criteria in \REF{ex:8-9}.

\ea\label{ex:8-9}
  Morphosyntactic criteria to distinguish the verbal affixes
...
\begin{table}
\caption{\label{tab:55}Verbal affixes distinguished by the morphosyntactic criteria}
\begin{tabularx}{\textwidth}{llllX}
\lsptoprule
            & A &  B &  C &  Examples\\
\midrule
\multicolumn{5}{l}{Inflectional affixes}\\
  \text{ } i. \text{ }\text{ }
  Direct inflections &  \surd &  \surd & \surd & \textit{-oo} (\textsc{int}), \textit{-ɨ} (\textsc{imp}), \textit{-na} (\textsc{proh}), \textit{-ɨba} (\textsc{sugs}), \textit{-azɨi} (\textsc{neg}.\textsc{plq}), \textit{-ba} (\textsc{csl}), \textit{-boo} (\textsc{cnd}), \textit{-tɨ} (\textsc{seq}), \textit{-təəra} ‘after’, \textit{-tai} (\textsc{lst}), \textit{-jagacinaa} (\textsc{sim}), \textit{-gadɨ} ‘until’\\

\tablevspace
 \text{ } ii. \text{ } 
 Indirect inflections &  \surd &  - & \surd & \textit{-i} (\textsc{npst}), \textit{-oo} (\textsc{infr}), \textit{-mɨ} (\textsc{plq}), \textit{-sa} (\textsc{pol}), \textit{-sɨga} (POL), \textit{-u} (\textsc{pfc}), \textit{-n} (\textsc{ptcp}), \textit{-tu} (\textsc{csl}), \textit{-too} (\textsc{csl}), \textit{-nən} (\textsc{seq})\\

\tablevspace
  \text{ } iii.  
  Bilateral inflections  &  - &  \surd & \surd & \textit{-an} (\textsc{neg}), \textit{-tar} (\textsc{pst}), \textit{-i}/\textit{-∅} (\textsc{inf})\\

\tablevspace
\midrule
\multicolumn{5}{l}{Derivational affixes}\\
  \text{ }i. \text{ }
  Connector derivations
  & - & - & \surd & \textit{-arɨr} (\textsc{pass}), \textit{-arɨr} (\textsc{cap}), \textit{-tur} (\textsc{prog}), \textit{-təər} (\textsc{rsl}), \textit{-jawur} (\textsc{pol}), \textit{-jur} (\textsc{act})\\

  \text{ }ii. 
  Non-connector derivations
...
Direct \& indirect inflections appear only in the word-final position (\ref{ex:8-9} A). Only direct inflections and bilateral inflections can finish a verb without another preceding affix (\ref{ex:8-9} B). As mentioned in the beginning of this chapter, the verbal form in the predicate determines the clausal type. In other words, all of the direct inflections, indirect inflections, and bilateral inflections are relevant to syntactic finiteness (\ref{ex:8-9} C). Additionally, connector derivations and the two bilateral inflections, i.e. \textit{-an} (\textsc{neg}) and \textit{-tar} (\textsc{pst})) are necessarily required by indirect inflections. Thus, those affixes are also relevant to syntactic finiteness. We will call the affixes which satisfy two or more criteria of \REF{ex:8-9} “inflectional affixes,” and the other remained affixes “derivational affixes” in the verbal morphology. It should be noted that the productivity among the above verbal affixes is not so much different from one another. For example, the derivational affix \textit{-jur} (act) can follow no less verbal roots than the inflectional affix \textit{-ɨ} (\textsc{imp}) can. Therefore, the term “derivational” does not imply less productivity, at least for verbal affixes, in this grammar.

Additionally, it is noteworthy that certain clitics are very similar to indirect inflections, i.e. the affix-like clitics (see \sectref{sec:4.2.2.2}): \textit{sɨ} (\textsc{fn}), \textit{doo} (\textsc{ass}), \textit{ka} (\textsc{dub}), \textit{kai} (\textsc{dub}), \textit{kamo} (\textsc{pos}), \textit{ga} (\textsc{cfm3}), and \textit{gajaaroo} (\textsc{dub}). These clitics fill the final slot of the verb, which is usually filled by inflectional affixes as in \REF{ex:8-1}, and the clitics cannot follow a verbal root directly (except for \textit{kai} (\textsc{dub})), and need one of the connector derivations in \REF{ex:8-5} in order for them to follow a verbal stem.

The morphophonology of verbs will be discussed in \sectref{sec:8.2}. The special types of verbal stems that have some morphological, syntactical, and semantical characteristics will be discussed in \sectref{sec:8.3}. The verbal inflectional morphology will be discussed in \sectref{sec:8.4}. The verbal derivational morphology will be discussed in \sectref{sec:8.5}.

\section{Morphophonology of verbs}
\label{sec:8.2}
\subsection{Rules for verbal roots and affixes}

In this section, we examine the morphophonological rules needed in order to correctly produce the output verbal forms. A complete list of the possible combinations of roots, derivational affixes, and inflectional affixes are shown in appendix. The morphophonological rule of \textit{-tar} (\textsc{pst}) and \textit{-mɨ} (\textsc{plq}) will be discussed in each section (see \sectref{sec:8.4.1.1} and \sectref{sec:8.4.1.4}).

Verbal affixes can be grouped into five (morphophonological) types as in \tabref{tab:56}, chiefly distinguished by their initial morphophonemes. The criteria to sub-categorize the verbal affixes in \tabref{tab:56} are based on morphophonological characteristics. Thus, the categorization is different from the one based on the morhposyntactic criteria in \REF{ex:8-9}. Each type of affixes needs a different set of (morpho)phonological rules to output the correct surface forms (see \sectref{sec:8.2.1.1} - \sectref{sec:8.2.1.4}).

\begin{table}[H]
\caption{\label{tab:56}Verbal affixes (and clitics) distinguished by the morphophonological criteria}
\begin{tabularx}{\textwidth}{llX}
Types  & Main characteristics  & All examples\\
\midrule
A. & vowel-initial & \textit{-an} (\textsc{neg}), \textit{-arɨr} (\textsc{pass}), \textit{-arɨr} (\textsc{cap}), \textit{-as} (\textsc{caus}), \textit{-azɨi} (\textsc{neg}.\textsc{plq}), \textit{-ɨ} (\textsc{imp}),\textit{-ɨba} (\textsc{sugs}), \textit{-oo}(\textsc{int}), \mbox{\textit{-oo} (\textsc{infr})}
\\

\tablevspace
B. &  \textit{t}-initial & \textit{-tar} (\textsc{pst}), \textit{-tuk} (\textsc{prpr}), \textit{-tur} (\textsc{prog}), \textit{-təər} (\textsc{rsl}), \textit{-tɨ} (\textsc{seq}), \textit{-tai} (\textsc{lst}), \textit{-təəra} ‘after’\\

\tablevspace
C. &  sonorant-initial & \textit{-jawur} (\textsc{pol}), \textit{-jaa} ‘person,’ \textit{-jur} (\textsc{act}), \mbox{\textit{-jagacinaa} (\textsc{sim})}, \textit{-mɨ} (\textsc{plq}), \textit{-n} (\textsc{ptcp}); \textit{sɨ} (\textsc{fn})\\

\tablevspace
D. & obstruent-initial & \textit{-ba} (\textsc{csl}), \textit{-boo} (\textsc{cnd}), \textit{-gadɨ} ‘until,’ \textit{-na} (\textsc{proh}), \textit{-sa} (\textsc{pol}), \textit{-sɨga} (POL), \textit{-too} (\textsc{csl}), \textit{-tu} (\textsc{csl}); \textit{doo} (\textsc{ass}) , \textit{ka} (\textsc{dub}), \textit{kai} (\textsc{dub}), \textit{kamo} (\textsc{pos}), \textit{ga} (\textsc{cfm3}), \textit{gajaaroo} (\textsc{dub})\\

\tablevspace
E. & infinitives & \textit{-i/-∅} (\textsc{inf})\\

\lspbottomrule
\end{tabularx}
\end{table}

The verbal stems are distinguished into 16 types, determined by their final morphophonemes (except for the irregular types). The types of verbal stems are shown in \tabref{tab:57} with a few examples.

\begin{table}[H]
\caption{\label{tab:57}16 types of verbal stems}
\begin{tabularx}{\textwidth}{llX}
\lsptoprule
No. &   Stem-final morphophonemes  & Examples\\
\midrule
1. & V\textsubscript{non-back}r;\text{ } uij
&  \textit{hingir-} ‘escape,’ \textit{abɨr-} ‘call,’ \textit{kəər-} \mbox{‘exchange,’} \textit{kuij-} ‘go beyond,’ \textit{buij-} ‘bark’\\

2. & V\textsubscript{back}r;\text{ } V\textsubscript{back}w                                 & \textit{tur-} ‘take,’ \textit{umuw-} ‘think,’ \textit{nuuw-} ‘sew,’ \textit{kˀuur-}/\textit{kˀuuw-} ‘close,’ \textit{nugoor-} \mbox{‘don’t do,’} \textit{koor-}/\textit{koow-}/\textit{kawur-} ‘buy,’ \textit{wa}(\textit{k})\textit{ar-} \mbox{‘understand’}\\

3. & pp                                                & \textit{app-} ‘play’  \\

4. & b;\text{ } nm                           & \textit{narab-} ‘line up,’ \textit{asɨb-} ‘paly,’ \textit{tanm-} ‘ask,’ \textit{cɨnm-} ‘wrap’ \\

5. & Vm                                                & \textit{jum-} ‘read,’ \textit{kam-} ‘eat,’ \textit{num-} ‘drink’ \\

6. & V\textsubscript{non-}\textit{\textsubscript{i}} k & \textit{kak-} ‘write,’ \textit{maruk-} ‘bandle \\

7. & V\textsubscript{non-}\textit{\textsubscript{i}} kk &  \textit{sukk-} ‘draw,’ \textit{mukk-} ‘bring’ \\

8. & Vs                                                 & \textit{us-} ‘push,’ \textit{kˀjoos-} ‘break’\\

9. &  ss                                               &
\textit{kuss-} ‘kill’\\

10. &  t                                                &     \textit{ut-} ‘hit,’ \textit{mat-} ‘wait,’ \textit{kat-} ‘win’\\

11. &  \$C(G)                                           &     \textit{jˀ-} ‘say,’\footnote{The word-initial glottalization of \textit{jˀ-} ‘say’ is frequently weakened to become /j/.} \textit{mj-} ‘see’\\

12. &  Cij                                              &     \textit{kij-} ‘cut,’ \textit{kij-} ‘put on (clothes),’ \mbox{\textit{kˀubij-} ‘tie,’} \textit{hasij-} ‘run’\\                                                     
13. &  V\textsubscript{non-}\textit{\textsubscript{i}} g &  \textit{tug-} ‘whet,’ \textit{hag-} ‘peel’  \\

14. &  ik                                               &     \textit{kik-} ‘hear,’ \textit{sik-} ‘spread’ \\

15. &  i(n)g                                            &     \textit{uig-} ‘swim,’ \textit{ming-} ‘grasp’ \\

16. &  in                                               &     \textit{sin-} ‘die,’ \textit{ikin-} ‘live’\\
...
Each type of verbal stem undergoes a different application of morphophonological rules according to the five types of verbal affixes (or clitics) in \tabref{tab:56}. The outputs in \tabref{tab:58} illustrate the different results caused by the applications of different morphophonological rules.

\begin{table} %size need to be fixed%
\caption{\label{tab:58}Different applications of rules to verbal stems and affixes showing their surface forms}
\small
\begin{tabular}{lllllll}
\lsptoprule
  & &  \multicolumn{5}{l}{Affix types}   \\
  & & Type A & Type B & Type C & Type D & Type E\\
  & & e.g. & e.g.& e.g.& e.g.& e.g.\\
\multicolumn{2}{l}{Stem types} &  \textit{-an} (\textsc{neg}) & \textit{-ta} (\textsc{pst}) & \textit{-jur} (\textsc{act}) & \textit{-na} (\textsc{proh}) & \textit{-i/-∅} (\textsc{inf}) \\
\midrule

1. & V\textsubscript{non-back}r;\text{ } uij  &  -an & \dagger -ta & \dagger -jur  & C\textit{\textsubscript{i}} -na &  -∅\\

2. & V\textsubscript{back}r;\text{ } V\textsubscript{back}w  &  -an &  \dagger -ta & \dagger -jur & C\textit{\textsubscript{i}} -na  &  -i \\

3. & pp &   -an & C\textit{\textsubscript{i}} \dagger -ta & -jur & -una  & -i \\

4. & b;\text{ } nm &   -an & \dagger -da  & -jur & -una & -i \\

5. & Vm  &  -an & \dagger -da & -jur & -na  & -∅  \\

6. &V\textsubscript{non-}\textit{\textsubscript{i}} k  &  -an & \dagger -cja  & -jur & -una  & -i\\

7. & V\textsubscript{non-}\textit{\textsubscript{i}} kk &   -an & C\textit{\textsubscript{i}} \dagger -cja & -jur & -una  &  -i\\

8. & Vs &   -an & \dagger -cja & -jur & -ɨna  & -i \\

9.& ss  &  -an & C\textit{\textsubscript{i}} \dagger -cja & -jur & -ɨna & -i \\

10. & t  &  -an & C\textit{\textsubscript{i}} -cja & c-jur & c-ɨna & -i \\

11. & \$C(G) &   -an & -icja & (\dagger)-jur & -uuna & -ii \\

12. & Cij  &  -an &  -cja & -jur & C\textit{\textsubscript{i}} -na  & -i \\

13. & V\textsubscript{non-}\textit{\textsubscript{i}} g  &  -an & \dagger -zja & -jur & -una & -i \\

14. & ik  &  -jan & \dagger -cja & -jur & -una  & -i\\

15. & i(n)g  &  -jan & \dagger -zja & -jur & -una  & -i\\

16. & in &  -jan & \dagger -zja & -jur & -na & -∅\\
...
(a) “\dagger” indicates the deletion of a morphophoneme before the morpheme boundary;
...
\tabref{tab:58} shows that each stem has a different set of outputs. Thus, I propose that there are 16 types of verbal stems (from the morphophonological perspective).

There are, however, some verbal stems that do not conform to the regular (morpho)phonological rules. For example, these stems include the light verb \textit{sɨr-} ‘do,’ the deictic motion verbs \textit{ik-} ‘go,’ \textit{k-} ‘come,’ and \textit{tɨkk-} ‘bring,’ the honorific verbs \textit{umoor-} (move.\textsc{hon}), \textit{misjoor-} (eat.\textsc{hon}), \textit{moor-} (\textsc{hon}), \textit{taboor-} (give.\textsc{hon}), and \textit{moosɨr-} (die.\textsc{hon}), the verbal roots ending with //aw// (such as \textit{hijaw-} ‘pick up,’ \textit{waraw-} ‘laugh,’ and \textit{juraw-} ‘gather’), and others such as \textit{sij-} ‘know,’ \textit{jurukub-} ‘happy,’ and \textit{hənk-} ‘enter.’ The (non-)application of the (morpho)phonological rules are summarized in \tabref{tab:59} (for the actual surface forms, see appendix).

\begin{table}
\caption{\label{tab:59}. Irregular type verbal stems}
\begin{tabular}{lllllll}
\lsptoprule
 & & \multicolumn{5}{l}{Affix types}\\
 
\multicolumn{2}{l}{Irregular stems} & Type A & Type B & Type C & Type D & Type E\\
\midrule

a.&  \textit{hijaw-} ‘pick up’ & IR & - & IR & IR & IR \\

b.&  \textit{k-} ‘come’  &  IR  &  IR  & - & IR & - \\

c.&  \textit{sɨr-} ‘do’ & -  & IR & IR & - & IR \\

d.&  \textit{hənk-} ‘enter’  & IR & IR & - & - & - \\

e.&  \textit{ik-} ‘go’ & - & IR & - & - & - \\

f.&  \textit{umoor-} (move.\textsc{hon}) &  - & IR & - & - & - \\

g.&  \textit{sij-} ‘know’& - & IR & - & - & - \\

h.&  \textit{jurukub-} ‘happy’  &  - & - & - & IR & - \\
...
\ex \textit{kik-}  ‘hear’   +  \textit{-an} (\textsc{neg})     >  /kik-jan/
\z
\z
...
There are nine verbal affixes that cause \textit{j}-insertion: \textit{-an} (\textsc{neg}), \textit{-arɨr} (\textsc{pass}), \textit{-arɨr} (\textsc{cap}), \textit{-as} (\textsc{caus}), \textit{-azɨi} (\textsc{neg}.\textsc{plq}), \textit{-ɨ} (\textsc{imp}), \textit{-ɨba} (\textsc{sugs}), \textit{-oo}(\textsc{int}), and \textit{-oo} (\textsc{infr}). These affixes will be called “vowel-initial affixes” (or “Type-A verbal affixes”). It should be mentioned that the infinitival affix \textit{-i} (\textsc{inf}) begins with a vowel, but its morphophonological behavior is different (see \sectref{sec:8.2.1.5}). The rule in \REF{ex:8-11} applies to Type-A verbal affixes.

\ea\label{ex:8-11} Morphophonological rule applied to Type-A verbal affixes

1. Insertion

\leftskip 0.1in
\parindent -0.1in
\text{ } If the nucleus of the stem-final syllable is //i//, /j/ is inserted into the initial position of Type-A verbal affix.

\text{ } (However, if the stem-final consonant is //r// or //j//\footnote{For the explanation of the restriction of //r//, see note (b) of the \tabref{tab:57}. Stem-final //j// prohibits the \textit{j}-insertion because it would make the /jj/ sequence, which never appears in Yuwan.}, the above rule does not apply.)

\z

The rule application and the output forms are shown in \tabref{tab:60}. In the following tables, the hyphen “-” in the cells means non-application of the rules.

\begin{table}[H] % Need to resize the table (especially the right side blank) %
\caption{\label{tab:60}Verbal stems + \textit{-an} (\textsc{neg})}
\begin{tabularx}{\textwidth}{lll}
\lsptoprule
Stem No. & Example/Input & 1. Insertion\\
\midrule
\text{ }1. & \Highlight{abɨr-an} (call-\textsc{neg}) & - \\

\text{ }2. & \Highlight{tur-an} (take-\textsc{neg}) & - \\

\text{ }3. & \Highlight{app-an} (play-\textsc{neg}) & - \\

\text{ }4. & \Highlight{narab-an} (line.up-\textsc{neg}) & - \\

\text{ }5. & \Highlight{jum-an} (read-\textsc{neg}) & - \\

\text{ }6. & \Highlight{kak-an} (write-\textsc{neg}) & - \\

\text{ }7. & \Highlight{sukk-an} (pull-\textsc{neg}) & - \\

\text{ }8. & \Highlight{us-an} (push-\textsc{neg}) & - \\

\text{ }9. & \Highlight{kuss-an} (kill-\textsc{neg}) & - \\

10. & \Highlight{ut-an} (hit-\textsc{neg}) & - \\

11. & \Highlight{mj-an} (see-\textsc{neg}) & - \\

12. & \Highlight{kij-an} (cut-\textsc{neg}) & - \\ 
                                                    
13. & \Highlight{tug-an} (whet-\textsc{neg}) & - \\

14. & kik-an (hear-\textsc{neg}) & \Highlight{kik-jan} \\

15. & uig-an (swim-\textsc{neg}) & \Highlight{uig-jan} \\

16. & sin-an (die-\textsc{neg}) & \Highlight{sin-jan} \\

\lspbottomrule
\end{tabularx}

\raggedright
Note: Output forms are underlined.

\end{table}

The affix \textit{-ɨba} (\textsc{sugs}) tends to become /ba/ after the verbal stems No. 5 and 16, e.g. \textit{jum-} ‘read’ + \textit{-ɨba} (SUGS) > /jumba/ (rather than /jumjɨba/) and \textit{sin-} ‘die’ + \textit{-ɨba} (SUGS) > /sinba/ (rather than /sinjɨba/). In addition, the combination of \textit{uig-} ‘swim’ and \textit{-ɨba} (SUGS) always becomes /uig-iba/ (not /uig-jɨba/).

\tabref{tab:60} shows that the verbal stems No. 14-16, which satisfy the conditions of the rule application discussed above, induce \textit{j}-insertion. In order to achieve simplicity with the above combination, we choose these output phonemes of the verbal stems as their underlying morphophonemes.

\subsubsection{Type B: Rules for \textit{t}-initial verbal affixes}
\label{bkm:Ref347175824}
The rules for affixes that begin with //t// are required in order to explain the following differences.

\ea\label{ex:8-12}
\gllll
    a. \textit{abɨr-}  ‘call’  +  \textit{-tɨ} (\textsc{seq})  >  /abɨ-tɨ/　\\
    b. \textit{jum-}  ‘read’   +  \textit{-tɨ}  (\textsc{seq})   >  /ju-dɨ/　\\
    c. \textit{kak-}  ‘write’  +  \textit{-tɨ}  (\textsc{seq})    >  /ka-cjɨ/ \\
    d. \textit{sin-}  ‘die’    +  \textit{-tɨ} (\textsc{seq})   >  /si-zjɨ/ \\
\z
...
There are seven verbal affixes that cause the above alternations: \textit{-tar} (\textsc{pst}), \textit{-tuk} (\textsc{prpr}), \textit{-tur} (\textsc{prog}), \textit{-təər} (\textsc{rsl}), \textit{-tɨ} (\textsc{seq}), \textit{-tai} (\textsc{lst}), and \textit{-təəra} ‘after.’ These affixes are called “\textit{t}-initial affixes” (or “Type-B verbal affixes”) because they all begin with //t//. It should be mentioned, however, that there are two affixes that begin with //t// but do not conform to the rules in this section, i.e. \textit{-tu} (\textsc{csl}) and \textit{-too} (\textsc{csl}) discussed in \sectref{sec:8.4.3.1}. If there is a combination of a verbal stem and a \textit{t}-initial affix, the five rules in \REF{ex:8-13} are applied in the order. The rule application and the output forms are shown in \tabref{tab:61}.

\ea\label{ex:8-13} Mophophonological rules applied to Type-B verbal affixes

1. Insertion

\leftskip 0.1in
\parindent -0.1in
\text{ } If the stem only contains consonants, //i// is inserted after the stem.

2. Affrication (palatalization)

\leftskip 0.1in
\parindent -0.1in
\text{ } If the stem has the vowel //i// in its final syllable (and the final consonant is not //r//), or if the stem-final morphophoneme is //t, s, k, g//, the initial //t// of the \textit{t}-initial verbal affix becomes //cj//.

3. Voicing

\leftskip 0.1in
\parindent -0.1in
\text{ } If the stem ends with //b, g, m, n//, the initial consonant of the \textit{t}-initial verbal affix is voiced.

4. Deletion

\leftskip 0.1in
\parindent -0.1in
\text{ } The final consonant (except for //t//) of the stem is deleted.

5. Assimilation

\leftskip 0.1in
\parindent -0.1in
\text{ } If the stem ends with a non-nasal consonant, it is assimilated with the following consonant.

\z


\begin{table}[H]
\caption{\label{tab:61} Verbal stems + \textit{-tɨ}  (\textsc{seq})}
\small
\begin{tabularx}{\textwidth}{llllll}
\lsptoprule
Stem No. \& Example/Input & 1. Inser & 2. Affr & 3. Voic & 4. Dele & 5. Assim\\
\midrule
\text{ }1. abɨr-tɨ (call-\textsc{seq}) & - & - & - & \Highlight{abɨ-tɨ} & - \\

\text{ }2. tur-tɨ (take-\textsc{seq}) & - & - & - & \Highlight{tu-tɨ} & - \\

\text{ }3. app-tɨ (play-\textsc{seq}) & - & - & - & ap-tɨ & \Highlight{at-tɨ} \\

\text{ }4. narab-tɨ (line.up-\textsc{seq}) & - & - & narab-dɨ & \Highlight{nara-dɨ} & - \\

\text{ }5. jum-tɨ (read-\textsc{seq}) & - & - & jum-dɨ & \Highlight{ju-dɨ} & - \\

\text{ }6. kak-tɨ (write-\textsc{seq}) & - & kak-cjɨ & - & \Highlight{ka-cjɨ} & - \\

\text{ }7. sukk-tɨ (pull-\textsc{seq}) & - & sukk-cjɨ & - & suk-cjɨ & \Highlight{suc-cjɨ} \\

\text{ }8. us-tɨ (push-\textsc{seq}) & - & us-cjɨ  & - & \Highlight{u-cjɨ} & - \\

\text{ }9. kuss-tɨ (kill-\textsc{seq}) & - & kuss-cjɨ  & - & kus-cjɨ & \Highlight{kuc-cjɨ} \\

10. ut-tɨ (hit-\textsc{seq}) & - & ut-cjɨ  & - & - & \Highlight{uc-cjɨ} \\

11. mj-tɨ (see-\textsc{seq}) & mji-tɨ & \Highlight{mji-cjɨ}  & - & - & - \\

12. kij-tɨ (cut-\textsc{seq}) & - & kij-cjɨ  & - & \Highlight{ki-cjɨ} & - \\                                                     
13. tug-tɨ (whet-\textsc{seq}) & - & tug-cjɨ  & tug-zjɨ & \Highlight{tu-zjɨ}  & - \\

14. kik-tɨ (hear-\textsc{seq}) & - & kik-cjɨ  & - & \Highlight{ki-cjɨ} & - \\

15. uig-tɨ (swim-\textsc{seq}) & - & uig-cjɨ  & uig-zjɨ & \Highlight{ui-zjɨ}  & - \\

16. sin-tɨ (die-\textsc{seq}) & - & sin-cjɨ  & sin-zjɨ & \Highlight{si-zjɨ} & - \\
...
It should be noted that the above rules do not apply to the negative affix \mbox{\textit{-an}} (\textsc{neg}). All of the “\textit{t}-initial affixes” can follow \textit{-an} (\textsc{neg}) without any morphophonological change, e.g. \textit{-an-tɨ} (\textsc{neg}-\textsc{seq}) becomes /-an-tɨ/ (not /-a-dɨ/) as in \REF{ex:8-104} in \sectref{sec:8.4.3.5}.


\subsubsection{Type C: Rules for sonorant-initial affixes (and clitics)}
\label{bkm:Ref347177215}
There are affixes and clitics that begin with sonorants: \textit{-jawur} (\textsc{pol}), \textit{-jaa} ‘person,’ \textit{-jur} (\textsc{act}), \textit{-jagacinaa} (\textsc{sim}), \textit{-mɨ} (\textsc{plq}), \textit{-n} (\textsc{ptcp}), and \textit{jaa} (\textsc{sol}), and which are called “sonorant-initial affixes (and clitics)” (or “Type-C verbal affixes (and clitics)”). It should be mentioned that the clitic \textit{sɨ} (\textsc{fn}) does not begin with a sonorant, but it still belongs to the Type-C verbal affixes. The rule in \REF{ex:8-14} applies to Type-C verbal affixes.

\ea\label{ex:8-14} Mophophonological rule applied to Type-C verbal affixes

1. Deletion

\leftskip 0.1in
\parindent -0.1in
\text{ } If the stem-final morphophoneme is a sonorant (except for nasals), the sonorant is deleted.

\z

The rule application and the output forms are shown in \tabref{tab:62}.

\begin{table}[H] % Need to resize the table (especially the right side blank) %
\caption{\label{tab:62}Verbal stems + \textit{-jur} (\textsc{act})}
\begin{tabularx}{\textwidth}{lll}
\lsptoprule
Stem No. & Example/Input & 1. Deletion\\
\midrule
\text{ }1. & abɨr-jur (call-\textsc{act}) & \Highlight{abɨ-jur} \\

\text{ }2. & tur-jur (take-\textsc{act}) & \Highlight{tu-jur} \\

\text{ }3. & \Highlight{app-jur} (play-\textsc{act}) & - \\

\text{ }4. & \Highlight{narab-jur} (line.up-\textsc{act}) & - \\

\text{ }5. & \Highlight{jum-jur} (read-\textsc{act}) & - \\

\text{ }6. & \Highlight{kak-jur} (write-\textsc{act}) & - \\

\text{ }7. & \Highlight{sukk-jur} (pull-\textsc{act}) & - \\

\text{ }8. & \Highlight{us-jur} (push-\textsc{act}) & - \\

\text{ }9. & \Highlight{kuss-jur} (kill-\textsc{act}) & - \\

10.\footnote{\text{ }The stem-final //t// becomes /c/ before //j// because of the phonological rule in \sectref{sec:2.4.2}.} & \Highlight{ut-jur} (hit-\textsc{act}) & - \\

11.\footnote{\text{ }There is a case where the stem-final //jˀ// is not deleted in order to retain the original root form, and the affix-initial //j// is deleted instead. That is, //jˀ-jur// becomes /∅-jur/ or /jˀ-ur/.} & mj-jur (see-\textsc{act}) & \Highlight{m-jur} \\

12. & kij-jur (cut-\textsc{act}) & \Highlight{ki-jur} \\ 

13. & \Highlight{tug-jur} (whet-\textsc{act}) & - \\

14. & \Highlight{kik-jur} (hear-\textsc{act}) & - \\

15. & \Highlight{uig-jur} (swim-\textsc{act}) & - \\

16. & \Highlight{sin-jur} (die-\textsc{act}) & - \\
...
Only the affix \textit{-jagacinaa} (\textsc{sim}) requires an additional rule of lengthening; i.e. it becomes /jaagacinaa/ after a verbal root containing only consonant(s). See \mbox{\tabref{tab:63}}.


\begin{table}[H] % Need to resize the table (especially the right side blank) %
\caption{\label{tab:63}Verbal stems + \textit{-jagacinaa} (\textsc{sim})}
\begin{tabularx}{\textwidth}{llll}
\lsptoprule
Stem No. & Example/Input & 1. Deletion & 2. Lengthening\\
\midrule

11. & mj-jagacinaa (see-\textsc{sim}) & m-jagacinaa & \Highlight{m-jaagacinaa}\\ 

11. & jˀ-jagacinaa (say-\textsc{sim}) & jˀ-agacinaa & \Highlight{jˀ-aagacinaa}\footnote{\text{ }Stem-final //jˀ// is not deleted in order to retain the original root form; instead, the affix-initial //j// is deleted.} \\

\midrule
\text{ }5. & \Highlight{jum-jagacinaa} (read-\textsc{sim}) & - & -\\

12. & kij-jagacinaa (cut-\textsc{sim}) & \Highlight{ki-jagacinaa} & -\\
...
\subsubsection{Type D: Rules for obstruent-initial affixes (and clitics)}
\label{bkm:Ref347177096}
It is necessary to derive rules in order to explain the following differences.

\ea\label{ex:8-16}
\gllll
    a. \textit{jum-}  ‘read’  +  \textit{-na} (\textsc{proh})  >  /jum-na/\\
    b. \textit{abɨr-}  ‘call’ +  \textit{-na} (\textsc{proh})  >  /abɨn-na/\\
    c. \textit{kak-}  ‘write’ +  \textit{-na} (\textsc{proh})  >  /kak-una/\\
    d. \textit{us-}  ‘push’  +  \textit{-na} (\textsc{proh})   >  /us-ɨna/\\
\z

The first example shows a simple combination of \textit{jum-} ‘read’ + \textit{-na} (\textsc{proh}) > /jumna/, but the next three require either nasal assimilation or vowel-insertion at the morpheme boundary. The verbal affixes that require these rules include \textit{-ba} (\textsc{csl}), \textit{-boo} (\textsc{cnd}), \textit{-gadɨ} ‘until,’ \textit{-sa} (\textsc{pol}), \textit{-sɨga} (POL), \textit{-tu} (\textsc{csl}), and \textit{-too} (\textsc{csl}). In addition, some “affix-like clitics” (see \sectref{sec:4.2.2.2}) are subject to the same rules, i.e. \textit{doo} (\textsc{ass}), \textit{ka} (\textsc{dub}), \textit{kai} (\textsc{dub}), \textit{kamo} (\textsc{pos}), \textit{ga} (\textsc{cfm3}), and \textit{gajaaroo} (\textsc{dub}). They are called 
“obstruent-initial affxies (and clitics)” (or “Type-D verbal affixes (and clitics)”). It should be pointed out, however, that the affix \textit{-na} (\textsc{proh}) does not begin with an obstruent, but it belongs to the Type-D verbal affixes.


If a verbal stem is combined with the Type-D affixes (or clitics), five rules should be applied in the order in \REF{ex:8-17}. It should be pointed out that if two rules have the same number, such as (3a) and (3b), the order of the rules is not significant, and either one of them is applied to the relevant stems.
It should be pointed out that if \textit{kai} (\textsc{dub}) or \textit{kamo} (\textsc{pos}) follows \textit{-tar} (\textsc{pst}), these rules do not apply and they simply delete the //r// of \textit{-tar} (\textsc{pst}) (see \sectref{sec:8.4.1.1}). The rule application and the output forms are shown in \tabref{tab:64}.


\ea\label{ex:8-17} Mophophonological rules applied to Type-D verbal affixes (and clitics)

1. Affrication

\leftskip 0.1in
\parindent -0.1in
\text{ } If the stem-final morphophoneme is //t//, it becomes //c/.\\

2a. Insertion

\leftskip 0.1in
\parindent -0.1in
\text{ }If the stem-final morphophoneme is a consonant after a syllable boundary, or is a non-sonorant consonant, //u// is inserted before the affix.\\

2b. Assimilation

\leftskip 0.1in
\parindent -0.1in
\text{ } If the stem-final morphophoneme is a non-nasal sonorant, it is assimilated to the following affix-initial consonant.\\

3a. Lengthening\footnote{\text{ }The stems preceding Type-D affixes seem to behave as if they were phonological words since they become bimoraic like many of the phonological words in Yuwan (cf. \sectref{sec:2.3.1}).}

\leftskip 0.1in
\parindent -0.1in
\text{ } If the stem originally contains only consonants, the inserted vowel //u// becomes /uu/.\\

3b. Centralizing

\leftskip 0.1in
\parindent -0.1in
\text{ } If the final morphophoneme of the stem is //c, s//, the following //u// becomes /ɨ/.\\

\z


\begin{table}[H]
\caption{\label{tab:64} Verbal stems + \textit{-na}  (\textsc{proh})}
\small
\begin{tabularx}{\textwidth}{llllll}
\lsptoprule
Stem No. \& Example/Input & 1. Affr & 2a. Inser & 2b. Assim & 3a. Leng  & 3b. Cent\\
\midrule
\text{ }1. abɨr-na (call-\textsc{proh}) & - & - & \Highlight{abɨn-na} & - & - \\

\text{ }2. tur-na (take-\textsc{proh}) & - & - & \Highlight{tun-na} & - & - \\

\text{ }3. app-na (play-\textsc{proh}) & - & \Highlight{happ-una} & - & - & - \\

\text{ }4. narab-na (line.up-\textsc{proh}) & - & \Highlight{narab-una} & - & - & - \\

\text{ }5. \Highlight{jum-na} (read-\textsc{proh}) & - & - & - & - & - \\

\text{ }6. kak-na (write-\textsc{proh}) & - & \Highlight{kak-una} & - & - &  - \\

\text{ }7. sukk-na (pull-\textsc{proh}) & - & \Highlight{sukk-una} & - & - & - \\

\text{ }8. us-na (push-\textsc{proh}) & - & us-una  & - & - & \Highlight{us-ɨna} \\

\text{ }9. kuss-na (kill-\textsc{proh}) & - & kuss-una  & - & - & \Highlight{kuss-ɨna} \\

10. ut-na (hit-\textsc{proh}) & uc-na & uc-una  & - & - & \Highlight{uc-ɨna} \\

11. mj-na (see-\textsc{proh}) & - & mj-una  & - & \Highlight{mj-uuna} & - \\

12. kij-na (cut-\textsc{proh}) & - & - & \Highlight{kin-na} & - & - \\                                                     
13. tug-na (whet-\textsc{proh}) & - & \Highlight{tug-una} & - & - & - \\

14. kik-na (hear-\textsc{proh}) & - & \Highlight{kik-una} & - & - & - \\

15. uig-na (swim-\textsc{proh}) & - & \Highlight{uig-una} & - & - & - \\

16. \Highlight{sin-na} (die-\textsc{proh}) & - & - & - & - & - \\
...
\subsubsection{Type E: Rules for the infinitival affixes}

Yuwan has the infinitival affixes (or type-E affixes) \textit{-i} and \textit{-∅}, which are selected by morphophonological conditions depending on the preceding verbal stem types (see \REF{ex:8-106} and \tabref{tab:65}). In addition, the surface forms of infinitives have two different forms depending on their morphosyntactic conditions. The two different surface forms are called basic forms and clause-final forms respectively. In principle, clause-final forms are used when the infinitives are clause-final free forms. Otherwise, i.e., when they are morphologically complex (e.g. compounding or affixation) or followed by another free form (e.g. copula verbs), basic forms are used. In most cases, the clause-final forms are vowel-lengthened basic forms. For example, the infinitive \textit{abɨ-∅} (call-\textsc{inf}) can be realized as /abɨ/ (basic form) or /abɨɨ/ (clause-final form). I will show the examples used in sentences; clause-final forms in \REF{ex:8-108-a} and basic forms in \REF{ex:8-108-b}.
...
\ea  Followed by a clitic

\glll  abɨn sjanmundoo.\\
\textit{abɨr-∅=n}  \textit{sɨr-tar-n=mun=doo}\\
call\textsc{-inf=even}  do\textsc{-pst-ptcp=advrs=ass}\\
\glt ‘Although (we) call (him), (he didn't come).’ [El: 110917]


\ex  Followed by the copula verb

\glll  minnasjɨ  abɨ  jataroo.\\
\textit{minna=sjɨ}  \textit{abɨr-∅}  \textit{jar-tar-oo}\\
everybody=\textsc{inst}  call-\textsc{inf}  \textsc{cop}-\textsc{pst}-\textsc{infr}\\
\glt ‘Probably (they) called (him) together.’ [El: 130814]


\ex  Followed by an affix

\glll  abɨmaidoo.\\
\textit{abɨr-∅-mai=doo}\\
call-\textsc{inf-obl=ass}\\
\glt ‘(We) have to call (that person).’ [El: No record of date]
...
\noindent
In (\ref{ex:8-108-a}a-b), the infinitives \textit{abɨr-∅} (call-\textsc{inf}) are clause-final free forms. Thus, they take the clause-final form /abɨɨ/. In contrast, in (\ref{ex:8-108-b}a-c), the infinitives \textit{abɨr-∅} (call-\textsc{inf}) are not the clause-final free forms, so they take the basic form /abɨ/. It should be remarked that the infinitive in (\ref{ex:8-108-b}c) is in the clause-final position, but that the word formation, i.e. the infinitival stem followed by the nominalizer affix \textit{-mai} (\textsc{obl}), shows that the infinitival stem itself is not a free form, so it cannot take the “clause-final form” (*/abɨɨ(mai)/), but takes the basic form (/abɨ(mai)/).\footnote{\text{ }In principle, the infinitive takes the clause-final form if it is a clause-final free form as in (\ref{ex:8-108-a}a-b), but there is a case where the infinitive which is a clause-final free form does not take the clause-final form, but takes the basic form as in (\ref{ex:8-114}a) in \sectref{sec:8.4.4.2.}. This phenomenon should be investigated in the future research.}

First, I will show the morphophonological rules for the basic infinitival forms in \REF{ex:8-106}.

\ea\label{ex:8-106}
  Mophophonological rules for basic infinitival forms
  
  1. Selection
  
\leftskip 0.1in
\parindent -0.1in
\text{ } If the verbal stem satisfies the following criteria (a) and (b), it takes \textit{-∅} (\textsc{inf}); otherwise, it takes \textit{-i} (\textsc{inf}).
  

\leftskip 0.1in
\parindent -0.1in
\text{ } a. The verbal stem belongs to 1, 5, or 16;

\leftskip 0.1in
\parindent -0.1in
\text{ } b. There is no possibility to form /C.C./ \\
...
\leftskip 0.1in
\parindent -0.1in
\text{ } //r// before \textit{-∅} (\textsc{inf}) and //j// before \textit{-i} (\textsc{inf}) are deleted.
\\
...
\leftskip 0.1in
\parindent -0.1in
\text{ } If the infinitive has only one mora in itself, its final vowel is lengthened. \\

\z

The second rule in (\ref{ex:8-106}) is required to explain the following behavior; \textit{kij-i} (cut-\textsc{inf}) + \textit{ja} (\textsc{top}) > /ki-i=ja/ (not */ki-jəə/), where the topic marker is never fused with the preceding morphophoneme (see also \sectref{sec:10.1.1.1}).

The above rules in (\ref{ex:8-106}) show that almost all of the infinitives take infinitival affix /-i/, and a few verbal stems (that end with non-back vowels or nasals) take the zero-form infinitival affix /-∅/. The infinitives made of each verbal stem type are presented in the following table, where the zero-form infinitival affix -∅ is expressed even in the output (surface) forms. The basic infinitival forms of all kinds of verbal stems are exemplified in \tabref{tab:65}.

\begin{table}
\caption{\label{tab:65}Infinitives: basic forms}
\begin{tabularx}{\textwidth}{llll}
\lsptoprule
Stem No. & Example/Input; 1. Selection & 2. Deletion & 3. Lengthening \\
\midrule
\text{ }1. & abɨr-∅ (call-\textsc{inf}) & \Highlight{abɨ-∅}  & - \\

\text{ }2. & tur-i (take-\textsc{inf}) \footnote{\text{ }The stem-final //r// is deleted because of the phonological rule in \sectref{sec:2.4.1}, and the output form is /tu-i/.} & - & - \\

\text{ }3. & \Highlight{app-i} (play-\textsc{inf}) & - & - \\

\text{ }4. & \Highlight{narab-i} (line.up-\textsc{inf}) & - & - \\

\text{ }5. & \Highlight{jum-∅/jum-i} (read-\textsc{inf}) \footnote{\text{ }The stem-final /m/ becomes [ɲ] or [ɴ] if it is followed by copula verbs; for example, /kam jappa/ [kaɲ.jap.pa]\textasciitilde[kaɴ.jap.pa] \textit{kam-∅ jar-ba} (eat-\textsc{inf} \textsc{cop-csl}) ‘because (we) eat (it).’}  & - & - \\

\text{ }6. & \Highlight{kak-i} (write-\textsc{inf}) & - & - \\

\text{ }7. & \Highlight{sukk-i} (pull-\textsc{inf}) & - & - \\

\text{ }8. & \Highlight{us-i} (push-\textsc{inf}) & - & - \\

\text{ }9. & \Highlight{kuss-i} (kill-\textsc{inf}) & - & - \\

10. & ut-i (hit-\textsc{inf}) \footnote{\text{ }The stem-final //t// becomes /c/ because of the phonological rule in \sectref{sec:2.4.2}, and the output form is /uc-i/.} & - & - \\

11. & mj-i (see-\textsc{inf}) & m-i & \Highlight{m-ii} \\

12. & kij-i (cut-\textsc{inf}) & \Highlight{ki-i} & - \\ 

13. & \Highlight{tug-i} (whet-\textsc{inf}) & - & - \\

14. & \Highlight{kik-i} (hear-\textsc{inf}) & - & - \\

15. & \Highlight{uig-i} (swim-\textsc{inf}) & - & - \\

16. & \Highlight{sin-∅/sin-i} (die-\textsc{inf}) & - & - \\
...
Next, I will discuss the clause-final forms. The rules for the clause-final forms are summarized in \REF{ex:8-107}.

\ea\label{ex:8-107}
  Mophophonological rules for clause-final infinitival forms
  
  1. Selection
  
\leftskip 0.1in
\parindent -0.1in
\text{ } The verbal stem No. 1 takes \textit{-∅} (\textsc{inf}) and the other stems take \textit{-i} (\textsc{inf}); in addition, the verbal stem No. 5 can take \textit{-∅} if it does not occur word-finally.\\
...
\leftskip 0.1in
\parindent -0.1in
\text{ } If the infinitive has only one vowel at its final syllable, the vowel is lengthened. \\

\z

The clause-final infinitival forms of all kinds of verbal stems are exemplified in \tabref{tab:key:83}.

\begin{table}
\caption{\label{tab:key:83}Infinitives: clause-final forms}
\begin{tabularx}{\textwidth}{llll}
\lsptoprule
Stem No. & Example/Input; 1. Selection & 2. Deletion & 3. Lengthening \\
\midrule
\text{ }1. & abɨr-∅ (call-\textsc{inf}) & abɨ-∅ & \Highlight{abɨɨ-∅} \\

\text{ }2. & tur-i (take-\textsc{inf}) \footnote{\text{ }The stem-final //r// is deleted because of the phonological rule in \sectref{sec:2.4.1}} & - & \Highlight{tu-ii} \\

\text{ }3. & app-i (play-\textsc{inf}) & - & \Highlight{app-ii} \\

\text{ }4. & narab-i (line.up-\textsc{inf}) & - & \Highlight{narab-ii} \\

\text{ }5. & \Highlight{jum-∅}/jum-i (read-\textsc{inf}) \footnote{\textit{ }The stem-final /m/ becomes [n] if it is directly followed by the assertive clitic \textit{doo}; for example, /jum doo/ [jun.doː] \textit{jum-∅=doo} (read-\textsc{inf=ass}) ‘(I) will read (books).’} & - & \Highlight{jum-ii} \\

\text{ }6. & kak-i (write-\textsc{inf}) & - & \Highlight{kak-ii} \\

\text{ }7. & sukk-i (pull-\textsc{inf}) & - & \Highlight{sukk-ii} \\

\text{ }8. & us-i (push-\textsc{inf}) & - & \Highlight{us-ii} \\

\text{ }9. & kuss-i (kill-\textsc{inf}) & - & \Highlight{kuss-ii} \\

10. & ut-i (hit-\textsc{inf}) \footnote{\text{ }The stem-final //t// becomes /c/ because of the phonological rule in \sectref{sec:2.4.2}} & - & \Highlight{uc-ii} \\

11. & mj-i (see-\textsc{inf}) & m-i & \Highlight{m-ii} \\

12. & kij-i (cut-\textsc{inf}) & \Highlight{ki-i} & - \\ 

13. & tug-i (whet-\textsc{inf}) & - & \Highlight{tug-ii} \\

14. & kik-i (hear-\textsc{inf}) & - & \Highlight{kik-ii} \\

15. & uig-i (swim-\textsc{inf}) & - & \Highlight{uig-ii} \\
...
\noindent It was difficult to find the appropriate questions to let the speaker say the clause-final form of the verbal stem No. 16. Thus, \tabref{tab:key:83} excludes the example of No. 16. 

The two tables (\tabref{tab:65} and \tabref{tab:key:83}) show that the verbal stems ending with nasals (No. 5 and 16) have two types of surface forms, i.e. the basic forms /jum-∅/ or /jum-i/ on No. 5 and /sin-∅/ or /sin-i/ on No. 16, and the clause-final forms /jum-∅/ or /jum-ii/ on No. 5. The rules to select the two surface forms are simple. If the infinitival forms do not violate the (morpho)phonological rules in Yuwan, they can occur with \textit{-∅} (\textsc{inf}) (i.e., possible to have nasal ending); otherwise, they have to take \textit{-i} (\textsc{inf}).

As the first rule for basic forms in \REF{ex:8-106} declares, if the infinitive does not form the /C.C./ (not /C.C/) syllable structure, it takes zero-form affix. However, if the infinitive forms the /C.C./ syllable structure, it takes \textit{-i} (\textsc{inf}). I will show the examples in \tabref{tab:82}

\begin{table}[H]
\caption{\label{tab:82}Verbal stems that have two basic forms (Stem No. 5 \& 16)}
\small
\begin{tabularx}{\textwidth}{llllll}
\lsptoprule
Stem No. \& Examples & \textit{-mai} (\textsc{obl}) & & \textit{n} ‘also’ & \\
\midrule

\text{ }\text{ }5. \textit{jum-} ‘read’ & \textit{jum-∅-mai} & /jum.mai/ & \textit{jum-i=n} & /ju.min/ (*/jum.n/)\\ 

16. \textit{sin-} \text{ }\text{ }‘die’ & \textit{sin-∅-mai} & /sin.mai/ & \textit{sin-i=n} & /si.nin/ \text{ }(*/sin.n/)\\

\midrule
\text{ }\text{ }6. \textit{kak-} ‘write’ & \textit{kak-i-mai} & /ka.ki.mai/ & \textit{kak-i=n} & /ka.kin/ \\
...
\noindent
The nominalizer \textit{-mai} (\textsc{obl}) and the limiter particle \textit{n} ‘also’ can follow infinitives, but the former follows /jum-∅/ (read-\textsc{inf}) and the latter follows /jum-i/  (read-\textsc{inf}) depending on their phonological conditions. The syllable structure /jum.mai/ (read.\textsc{inf.obl}) is stable in the syllable structure in Yuwan, but /jum.n/ (read.\textsc{inf}.also) is not (i.e., not allowing /n/ as the nucleus in the syllable). Thus, the latter infinitive selected the infinitival affix \textit{-i}, and becomes /ju.min/.



%各表の注で記載した内容を以下に例文つきで説明するか？%


  In addition, \textit{doo} (\textsc{ass}) permits the verbal stem No. 5 (ending with //Vm//) to become not only the clause-final form, e.g. /jum-ii=doo/ (read-\textsc{inf}=\textsc{ass}), but also the basic form, e.g. /jum-∅=doo/ (read-\textsc{inf}=\textsc{ass}), even in the clause-final position. This alternation is not permitted before \textit{na} (\textsc{plq}), e.g. */jum-∅=na/ (read-\textsc{inf}=\textsc{pl}Q), where the verbal stem No. 5 always takes the clause-final form, e.g. /jum-ii=na/ (read-\textsc{inf}=\textsc{pl}Q) ‘Does (someone) read?’ It is probable that this restriction avoids the confusion between \textit{na} (\textsc{pl}Q) and \textit{-na} (\textsc{proh}), since the latter can form /jum-na/ (read-\textsc{proh}) ‘Don’t read!’

  Before concluding this section, it should be mentioned that the difference between the basic form and the clause-final form of infinitives may indicates an intonational unit. In other words, an infinitive would be lengthened if it is in the final position of the intonational unit. In that case, the clause-final particles, e.g. \textit{doo} (\textsc{ass}), seem to attach to the intonational unit. This analysis is in need of further resarch.
...
arɨr\textsubscript{ C}      \textit{-ɨba} (\textsc{sugs})      arɨɨ\textsubscript{ P/C}    \textit{sɨ}  (\textsc{fn})                                                                                                                                                                                                                                                                                                                                                                                                           \\
    ...
    ar\textsubscript{ P}  \textit{-oo} (\textsc{int})      arɨɨ\textsubscript{ P/C}    \textit{-mɨ} (\textsc{plq})                                                                                                                                                                                                                                                                                                                                                                                                              \\
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \\
arɨr\textsubscript{ C}  arɨɨr\textsubscript{ C}    \textit{-oo} (\textsc{infr})    arɨ\textsubscript{ P/C}      \textit{-n} (\textsc{ptcp})                                                                                                                                                                                                                                                                                                                                                                                     \\
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                \\
...
(b) The passive affix cannot precede \textit{-oo} (\textsc{infr}). The assumed meaning is expressed by the combination of \textit{-arɨr} (\textsc{pass}) + \textit{-∅} (\textsc{inf}) + \textit{daroo} (infr), e.g. /acjaa wanga utarɨdaro/ \textit{acja} \textit{wan=ga} \textit{ut-arɨr-∅=daroo} (tomorrow 1\textsc{sg}=\textsc{nom} hit-\textsc{pass}-\textsc{inf}=infr) ‘Probably, I will be hit tomorrow’;
...
Renyou            &  kakj & -u\textsubscript{1} (\textsc{act})  i (\textsc{npst}), ru (\textsc{pfc}), etc.                                                        \\
...
\begin{table}
\caption{\label{tab:key:69} Comparison of anaflyses by \citet{UchimaEtAl1976} and the present author (in surface forms)}
\begin{tabular}{ll ll}
\lsptoprule
...
\end{table}
  ...
  Furthermore, the analysis proposed by \citet{UchimaEtAl1976} creates unnecessary homophonic morphemes such as \textit{-ɨ} (\textsc{imp}) vs. \textit{-ɨ} (\textsc{seq}), and \textit{-u}\textsubscript{1} (\textsc{act}) vs. \textit{-u}\textsubscript{2} (\textsc{prog}). On the other hand, our analysis does not fall into this trap, e.g. \textit{-ɨ} (\textsc{imp}) vs. \textit{-tɨ} (\textsc{seq}), and \textit{-jur} (act) vs. \textit{-tur} (PROG).
...
2.\textsc{hon}.\textsc{sg}=\textsc{nom}  [speak-\textsc{seq}  \textsc{hon}-\textsc{act}-\textsc{ptcp}]  thing   hear-\textsc{inf}+want-\textsc{adj}+\textsc{stv}-\textsc{ptcp}=\textsc{qt}  say-\textsc{seq}\\
...
Tsune  front    first  go.\textsc{hon}-\textsc{csn}=\textsc{infr}=\textsc{qt}  think-\textsc{seq}=\textsc{foc}\\
...
Yoneshige-\textsc{pl}=\textsc{nom}  exist-\textsc{pst}-\textsc{ptcp}  house=\textsc{loc1}   exist.\textsc{hon}-\textsc{act}-\textsc{ptcp}=\textsc{cfp}\\
...
\textit{kunəəda}  \textit{tacuu=ga}  \textit{moosɨr-tar-oo=ga}\\
    the.other.day  Tatsu=\textsc{nom}  die.\textsc{hon}-\textsc{pst}-\textsc{infr}=\textsc{cfm3}\\
...
The auxiliary honorific verbs in \tabref{tab:key:73} need to be preceded by a lexical verb, and the lexical verb always takes \textit{-tɨ} (\textsc{seq}) (see \sectref{sec:9.1.1} for more details). \textit{moor-} (\textsc{hon}) is used just to add an honorific meaning to the preceding verb. In other words, \textit{moor-} (\textsc{hon}) is an auxiliary honorific verb that is semantically unmarked. In contrast, \textit{taboor-} (\textsc{ben}.\textsc{hon}) and \textit{umoor-} (come.\textsc{hon}) add other meanings besides the honorific meaning. First, I will present examples of \textit{moor-} (\textsc{hom}).
...
banyan.tree  [know-\textsc{seq}  \textsc{hon}-\textsc{act}]=\textsc{cfm1}=\textsc{sol}\\
...
2.\textsc{hon}-\textsc{pl}=\textsc{nom}  [do-\textsc{seq}  \textsc{hon}-\textsc{act}-\textsc{inf}]=\textsc{dat1}=\textsc{top}  island=\textsc{gen} fish=\textsc{infr}=\textsc{cfm3}\\
...
In (\ref{ex:8-35}a), the subject of the clause is \textit{sinsjei} ‘teacher,’ which is clear from the unacceptability of (\ref{ex:8-35}b). The difference between (\ref{ex:8-35}a) and (\ref{ex:8-35}b) is only on the subjects of the clauses (see also Chapter 3). In contrast, the difference between (\ref{ex:8-35}a) and (8-35c) is only on the core arguments immediately preceding the predicates, i.e. \textit{jɨɨi} ‘brother’ and \textit{kanɨ} ‘money.’ As mentioned before, the core argument of \textit{umoor-} (exist.\textsc{hon}) must indicate an animate referent. Thus, (\ref{ex:8-35}c) is ungrammatical since the core argument, i.e. \textit{kanɨ} ‘money,’ is inanimate. If we replace \textit{umoor-} (exist.\textsc{hon}) in (\ref{ex:8-35}c) with \textit{ar-tɨ} \textit{moor-} (exist-\textsc{seq} \textsc{hon}), which is a honorific expression of \textit{ar-} ‘exist’ (see \sectref{sec:8.3.2.2}), as in (\ref{ex:8-35}d), the sentence can be grammatical, since \textit{ar-} ‘exist’ may take an inanimate core argument. These examples show that the core argument of the existential verbs is sometimes different from the subject.
...
\ex \textit{-oo} (\textsc{infr})

{\TM}
\glll  ukka  cugəə,  mata,  (maga,)  maga  jaroo.\\
\textit{u-rɨ=ga}  \textit{cugi=ja}  \textit{mata}  \textit{maga}  \textit{maga}  \textit{jar-oo}\\
\textsc{mes}-\textsc{nlz}=\textsc{gen}  next=\textsc{top}  again  grandchild  grandchild  \textsc{cop}-\textsc{infr}\\
...
village.office=\textsc{top}  \textsc{cop}-\textsc{neg}-\textsc{seq}    prefectural.road=\textsc{infr}\\
...
\glll  miicɨdu  cigajurooga?\\
    ...
    three.thing=\textsc{foc}  different-\textsc{act}-\textsc{infr}=\textsc{cfm3}\\
...
\textit{wan=ga}  \textit{kik-tur-n=ccjɨ}  \textit{umuw-tɨ=du}  \textit{urattəə}   \textit{ga-n}  \textit{sɨr-tar-n}  \{[\textit{ar-an}]\textsubscript{Adnominal clause}  \textit{hanasi}\}\textsubscript{NP}  \textit{sɨr-tar-oo=ga=i}\\
    1\textsc{sg}=\textsc{nom}  hear-\textsc{prog}-\textsc{ptcp}=\textsc{qt}  think-\textsc{seq}=\textsc{foc}  2.\textsc{nhon}.\textsc{du}  \textsc{mes}-\textsc{adnz}  do-\textsc{pst}-\textsc{ptcp}  \textsc{cop}-\textsc{neg}  tale  do-\textsc{pst}-\textsc{infr}=\textsc{cfm}4=\textsc{plq}\\
...
Niijima-\textsc{hon}=\textsc{com}  father-\textsc{pl}=\textsc{nom}  brother  [\textsc{cop}-\textsc{seq}  \textsc{hon}-\textsc{act}]=\textsc{dub}\\
            {}[Lex. verb  Aux. verb]\textsubscript{VP}\\
...
\textsc{dist}-\textsc{adnz}  person=\textsc{top}  teacher  [\textsc{cop}-\textsc{seq}  \textsc{hon}-\textsc{act}-\textsc{npst}]=\textsc{plq}\\
            {}[Lex. verb  Aux. verb]\textsubscript{VP}\\
...
\textsc{mes}-\textsc{nlz}=\textsc{top}  good-\textsc{adj}  \textsc{stv}-\textsc{infr}=\textsc{cfm3}=\textsc{plq}\\
...
black-\textsc{adj}  \textsc{stv}-\textsc{neg}=\textsc{infr}=\textsc{cfm3}\\
...
Moreover, they fill different kinds of predicate phrases. The existential verb fills the verbal predicate phrase, the copula verb fills the nominal predicate phrase, and the stative verb fills the adjectival predicate phrase (see Chapter 9 for more details). Thus, these \textsc{ecs} verbs are different from one another. There are, however, a few similarities among them: (A) they can directly precede indirect inflections; (B) they choose the form /ar-/ in \textsc{avc}.

  First, in (\ref{ex:8-3}b) in \sectref{sec:8.1}, we have discussed a certain group of inflectional affixes, i.e. indirect inflections, which cannot directly follow any verbal root. However, \textsc{ecs} verbs can directly precede indirect inflections. For example, \textit{-i} (\textsc{npst}) and \textit{-oo} (\textsc{infr}) are members of indirect inflections, but they can follow the existential verbs directly.

\ea\label{ex:8-50}
  Existential verbs + Indirect inflections
...
\ex \textit{ar-} ‘exist (inanimate)’ + \textit{-oo} (\textsc{infr})

{\TM}
\glll  an,  namanu  {\textbar}jakkjoku{\textbar}nu  aroogai?\\
\textit{a-n}  \textit{nama=nu}  \textit{jakkjoku=nu}  \textit{ar-oo=ga=i}\\
\textsc{dist}-\textsc{adnz}  now=\textsc{gen}  pharmacy=\textsc{nom}  exist-\textsc{infr}=\textsc{cfm3}=\textsc{plq}\\
\glt ‘That (pharmacy), (i.e.) the pharmacy (that exists there) now probably (still) exists, right?’ [Co: 111113\_01.txt]
\z
\z

In (\ref{ex:8-50}a), \textit{wur-} ‘exist’ directly precedes \textit{-i} (\textsc{npst}). In (\ref{ex:8-50}b), \textit{ar-} ‘exist’ directly precedes \textit{-oo} (\textsc{infr}). It should be noted that \textit{-oo} (infr) has the same form with \textit{-oo} (\textsc{int}). They can usually be distinguished by their morphological environments, since the former belongs to indirect inflections, and the latter belongs to direct inflections, and direct inflections can follow verbal roots directly. However, the existential verb \textit{wur-} ‘exist’ can take indirect inflections directly. Thus, we cannot distinguish them by their morphological environments. The following examples show this case.

\ea\label{ex:8-51}
\ea \textit{wur-} ‘exist’ + \textit{-oo} (\textsc{infr})
...
house=\textsc{loc1}  exist-\textsc{infr}=\textsc{cfm1}\\
...
In (\ref{ex:8-51}a-b), we can distinguish \textit{-oo} (\textsc{infr}) from \textit{-oo} (\textsc{int}) only by the contexts. In contrast with \textit{wur-} ‘exist,’ another existential verb \textit{ar-} ‘exist’ cannot take animate subjets. Thus, it is difficult for \textit{ar-} ‘exist’ to take \textit{-oo} (INT), since \textit{-oo} (INT) expresses the subject’s intention (see \sectref{sec:8.4.1.2}). The example where the copula verb takes the Indirect inflection \textit{-oo} (infr) was shown in (\ref{ex:8-36}b) in \sectref{sec:8.3.3.1.} An example where the stative verb takes \textit{-oo} (infr) was shown in (\ref{ex:8-47}b) in \sectref{sec:8.3.4.1.}
...
\midrule
Finite-form affixes &  \textit{-oo} (\textsc{int}), \textit{-oo} (\textsc{infr}), \textit{-ɨ} (\textsc{imp}), \textit{-na} (\textsc{proh}), \textit{-ɨba} (\textsc{sugs}), \textit{-azɨi} (\textsc{neg}.\textsc{plq}),                                                                                                 \\
...
\ea\label{ex:8-52}
  Finite-form affixes
    ...
    \ex Mood\\
        \textit{-oo} (\textsc{int}) and \textit{-oo} (\textsc{infr})
    \ex Politeness\\
...
As mentioned in \sectref{sec:8.1}, the finite-form affixes can be separated into two groups, i.e. direct inflections or indirect inflections. Therefore, the finite-form affixes that belong to indirect inflections, i.e. \textit{-i} (\textsc{npst}), \textit{-oo} (\textsc{infr}), \textit{-mɨ} (\textsc{plq}), \textit{-sa} (\textsc{pol}), \textit{-sɨga} (POL), and \textit{-u} (\textsc{pfc}), cannot directly follow the verbal roots (with the exception of \textsc{ecs} verbs discussed in \sectref{sec:8.3.5}). A complete lists of the possible combinations of 16 types of verbal stems (see \sectref{sec:8.2.1}) and the finite-form affixes will be shown in appendix.
...
\ea\label{ex:8-53}
  Verbal morphemes that can directly precede \textit{-i} (\textsc{npst}) (Finite-form affix; Indirect inflection)
...
The finite-form affix \textit{-i} (\textsc{npst}) belongs to indirect inflections (see \sectref{sec:8.1}). Thus, it cannot directly follow any verbal root and always takes one of the affixes in \REF{ex:8-53} to close the word. I will present an example in \REF{ex:8-54}.
...
\textit{hoogen=nu}  \textit{attakəə}  \textit{wakar-jur-i}\\
    dialect=\textsc{nom}  everything  understand-\textsc{act}-\textsc{npst}\\
\glt ‘(He) understands everything (about our) dialect.’ [Co: 110328\_00.txt]
\z

  In contrast, \textit{-tar} (\textsc{pst}) can directly follow any verbal root as in \REF{ex:8-55}. I will present the verbal morphemes that can directly precede \textit{-tar} (\textsc{pst}) in \REF{ex:8-55}.

\ea\label{ex:8-55}
  Verbal morphemes that can directly precede \textit{-tar} (\textsc{pst}) (Finite-form affix; Direct inflection)
...
In principle, the affix-final //r// or \textit{-tar} (\textsc{pst}) assimilates to the initial consonant of Type-D affixes (or clitics) (see \sectref{sec:8.2.1.4}). However, \textit{-tar} (\textsc{pst}) becomes /ta/ (not /tak/) only before \textit{kai} (\textsc{dub}) or \textit{kamo} (\textsc{pos}).
...
\subsubsection{Mood: \textit{-oo} (\textsc{int}) and \textit{-oo} (\textsc{infr})}

The finite-form affixes \textit{-oo} (\textsc{int}) and \textit{-oo} (\textsc{infr}) express the mood. First, I will present the verbal morphemes that can directly predede \textit{-oo} (INT). The affixes deleted by double lines cannot directly precede the word-final affix.

\ea\label{ex:8-58}
  Verbal morphemes that can directly precede \textit{-oo} (\textsc{int}) (Finite-form affix; Direct inflection)
...
As mentioned before, \textit{-oo} (\textsc{int}) belongs to direct inflections, and it can directly follow the verbal roots as in (\ref{ex:8-59}a). It may also follow another verbal affix as in (\ref{ex:8-59}b-c).
...
The example (\ref{ex:8-59}c) contains the passive affix \textit{-ar}, and the verb as a whole expresses the intention of the subject (not the agent). In other words, \textit{-oo} (\textsc{int}) expresses the subject’s (not the agent’s) intention. The subject of the finite-form verb composed of \textit{-oo} (INT) is always the speaker.
  ...
  Secondly, \textit{-oo} (\textsc{infr}) belongs to indirect inflections. Thus, it cannot follow any verbal root directly.

\ea\label{ex:8-60}
  Verbal morphemes that can directly precede \textit{-oo} (\textsc{infr}) (Finite-form affix; Indirect inflection)

  Root  \textit{-as  -arɨr} %%[Warning: Draw object ignored]
\textit{-tuk  -arɨr  -tur  -jawur} %%[Warning: Draw object ignored]
\textit{-an  -təər  -tar  -oo} (\textsc{infr})
...
I will present examples of \textit{-oo} (\textsc{infr}) in (\ref{ex:8-61}a-b).
...
now=\textsc{gen}  cow  do-\textsc{act}-\textsc{infr}=\textsc{cfm3}\\
...
2.\textsc{hon}.\textsc{sg}=\textsc{nom}  fish+house  do-\textsc{act}-\textsc{pst}-\textsc{infr}=\textsc{cfm3}\\
\glt ‘You used to run a fish shop, didn’t you?’ [Co: 110328\_00.txt]
\z
\z

  Apparently, \textit{-oo} (\textsc{int}) and \textit{-oo} (\textsc{infr}) have the same form. Therefore, there are a few cases, where it is difficult to draw a distinction between the two affixes by their morphological environments, e.g. after “\textsc{ecs} verbs” (see \sectref{sec:8.3.5}) or after the derivational affix \textit{-tur} (\textsc{prog}) as in \REF{ex:8-62}.
...
Akira=\textsc{top}  \textsc{dist}-place=\textsc{loc}2  read-\textsc{prog}-\textsc{infr}\\
\glt ‘Probably, Akira is reading (the book) there.’ [El: 130820]
\z
\z

In these examples, we can distinguish \textit{-oo} (\textsc{int}) from \textit{-oo} (\textsc{infr}) only by the contexts (e.g. the subjects of the clauses).

\subsubsection{Politeness: \textit{-sa} (\textsc{pol}) and \textit{-sɨga} (POL)}

The finite-form affixes \textit{-sa} (\textsc{pol}) and \textit{-sɨga} (POL) are used to express politeness to the hearer. They belong to indirect inflections, so they cannot directly follow any verbal root. The verbal affixes that can directly precede \textit{-sa} (POL) and \textit{-sɨga} (POL) are almost the same, but only \textit{-an} (\textsc{neg}) cannot precede \textit{-sa} (POL) as in (\ref{ex:8-63}a). The affixes deleted by double lines cannot directly precede the word-final affix.

\ea\label{ex:8-63}
\ea Verbal morphemes that can directly precede \textit{-sa} (\textsc{pol}) (Finite-form affix; Indirect inflection)
...
\ex Verbal morphemes that can directly precede \textit{-sɨga} (\textsc{pol}) (Finite-form affix; Indirect inflection)
...
As mentioned in \sectref{sec:1.4.2}, the old people rarely use the derivational politeness affix \textit{-jawur}. In contrast, they use the inflectional politeness affix \textit{-sa} or \textit{-sɨga} as in (\ref{ex:8-64}a-c).
...
reply=\textsc{top}  do-\textsc{act}-\textsc{pol}\\
...
cement+bag=\textsc{top}  \textsc{dist}-\textsc{nlz}  thirty.kilogram=\textsc{qt}  say-\textsc{act}-\textsc{pst}-\textsc{pol}\\
...
The finite-form affixes \textit{-mɨ} (\textsc{plq}) and \textit{-azɨi} (\textsc{neg}.\textsc{pl}Q) express the polar question (i.e. “yes-no question”).   First, \textit{-mɨ} (\textsc{pl}Q) belongs to the indirect inflections, so it cannot directly follow any verbal root. The affixes deleted by double lines cannot directly precede the word-final affix.

\ea\label{ex:8-65}
  Verbal morphemes that can directly precede \textit{-mɨ} (\textsc{plq}) (Finite-form affix; Indirect inflection)
...
\textit{waakja-a}  \textit{jaa=nantɨ}  \textit{kam-jur-mɨ}\\
    1\textsc{pl}-\textsc{adnz}  house=\textsc{loc1}  eat-\textsc{act}-\textsc{plq}\\
...
\ea\label{ex:8-67}
  Verbal morphemes that can directly precede \textit{-azɨi} (\textsc{neg}.\textsc{plq}) (Finite-form affix; Direct inflection)
...
(Finite-form affixes; Direct inflection)
...
much  eat-\textsc{imp}=\textsc{cfm1}=\textsc{qt}=\textsc{foc}  say-\textsc{act}-\textsc{pst}-\textsc{csl}=\textsc{sol}\\
...
\ea\label{ex:8-75}
  Verbal morphemes that can directly precede \textit{-u} (\textsc{pfc}) (Finite-form affix; Indirect inflection)
...
\textit{tuu=nu}  \textit{ujubɨ=ja}  \textit{məə=kaci=du}  \textit{magar-jur-u}   \textit{usiju=kaci=ja}  \textit{magar-an=doo}\\
    ten=\textsc{gen}  finger=\textsc{top}  front=\textsc{all}=\textsc{foc}  bend-\textsc{act}-\textsc{pfc}  back=\textsc{all}=\textsc{top}  bend-\textsc{neg}=\textsc{ass}\\
...
The participial affix \textit{-n} (\textsc{ptcp}) belongs to indirect inflections (see \sectref{sec:8.1}), i.e. cannot directly follow the verbal roots, and takes one of the affixes in \REF{ex:8-79}. The affixes deleted by double lines cannot directly precede \textit{-n} (\textsc{ptcp}).

\ea\label{ex:8-79}
  Verbal morphemes that can directly precede \textit{-n} (\textsc{ptcp}) (Participial affix; Indirect inflection)
...
any.more  talk-\textsc{act}-\textsc{ptcp}  seed=\textsc{nom}  exist-\textsc{neg}-\textsc{csl}=\textsc{sol}\\
...
\textsc{dist}-\textsc{adnz}  Sae=\textsc{com}  Uyuri=only=\textsc{foc}  come-\textsc{act}-\textsc{ptcp}\\
...
\textit{kam-jur-n}  \textit{cikjara=du}  \textit{ar=doo}\\
    eat-\textsc{act}-\textsc{ptcp}  power=\textsc{foc}  exist=\textsc{ass}\\
\glt ‘If (you) eat (much), (you will have) power.’ [Co: 120415\_01.txt]
\z

In \REF{ex:8-81}, the participle /kamjun/ \textit{kam-jur-n} (eat-\textsc{act}-\textsc{ptcp}) functions like a converb meaning ‘if (you) eat (much).’ There is no other expression like \REF{ex:8-81} in Yuwan, so this saying seems to be a fossilized expression.
...
\ea\label{ex:8-82}
  Verbal morphemes that can directly precede \textit{-an} (\textsc{neg}) (Participial affix; Direct inflection)
...
\ex Verbal morphemes that can directly precede \textit{-tu} (\textsc{csl}) (Converbal affix; Indirect inflection)
...
\ex Verbal morphemes that can directly precede \textit{-too} (\textsc{csl}) (Converbal affix; Indirect inflection)
...
carrot=\textsc{nom}  exist-\textsc{csl}  \textsc{cop}-\textsc{neg}=\textsc{infr}\\
...
\textsc{mes}-\textsc{nlz}=\textsc{top}  again  who-NLZ-\textsc{sg}=\textsc{nom}  \textsc{cop}-\textsc{pst}=\textsc{dub}      come-\textsc{prog}-\textsc{pst}-\textsc{csl}  \textsc{mes}-\textsc{nlz}  past=\textsc{gen}  ten.nine=\textsc{gen}  spring=\textsc{top}  \textsc{prox}-\textsc{nlz}=\textsc{foc}  sing-\textsc{act}-\textsc{pst}-\textsc{csl}=\textsc{sol}=\textsc{qt}  say-\textsc{seq}\\
...
The converbal affix \textit{-boo} (\textsc{cnd}) fills the predicates of adverbial clauses that express the condition that can realize the event of the superordinate clause. \textit{-boo} (CND) belongs to direct inflections. Thus, it can directly follow a verbal root. Additionally, it can follow all of the derivational affixes and the inflectional affix \textit{-an} (\textsc{neg}), but cannot follow \textit{-tar} (\textsc{pst}) as in \REF{ex:8-89}.

\ea\label{ex:8-89}
  Verbal morphemes that can directly precede \textit{-boo} (\textsc{cnd}) (Converbal affix; Direct inflection)
...
mouth  wide-\textsc{advz}  open-\textsc{cnd}  artificial.tooth=\textsc{nom}  drop-\textsc{act}-\textsc{ptcp}=\textsc{csl}\\
...
\ea\label{ex:8-92}
  Verbal morphemes that can directly precede \textit{-tai} (\textsc{lst}) (Converbal affix; Direct inflection)
...
what-\textsc{indfz}  teach-\textsc{seq}  \textsc{ben}-\textsc{lst}  \textsc{mes}-\textsc{nlz}  do-\textsc{seq}  \textsc{ben}-\textsc{pst}-\textsc{ptcp}   person=\textsc{gen}  event=\textsc{top}  think-\textsc{inf}    forget-\textsc{neg}.\textsc{seq}  \textsc{mes}-\textsc{nlz} do-\textsc{act}-\textsc{ptcp}=\textsc{advrs}\\
...
\ea\label{ex:8-94}
  Verbal morphemes that can directly precede \textit{-gadɨ} ‘until’ (Converbal affix; Direct inflection)
...
2.\textsc{hon}.\textsc{pl}  come-until  exist-\textsc{pst}-\textsc{infr}=\textsc{cfm3}\\
...
\ea\label{ex:8-97}
  Verbal morphemes that can directly precede \textit{-jagacinaa} (\textsc{sim}) (Converbal affix; Direct inflection)
...
how-\textsc{advz}=\textsc{foc}  do-\textsc{act}-\textsc{pfc}=\textsc{qt}  2.\textsc{hon}-\textsc{du}=\textsc{appr}  talk-\textsc{sim}    this.year=\textsc{top}  New\_Year’s\_Day=\textsc{top}  \textsc{mes}-\textsc{nlz}  \textsc{cop}-\textsc{csl}  \textsc{mes}-\textsc{adnz}  New\_Year’s\_Day  \REF=\textsc{top}  cow-\textsc{dim}  buy-\textsc{int}=\textsc{sol}=\textsc{qt}  say-\textsc{seq}\\
...
\ea\label{ex:8-101}
\ea Verbal morphemes that can directly precede \textit{-tɨ} (\textsc{seq}) (Converbal affix; Direct inflection)
...
\ex Verbal morphemes that can directly precede \textit{-nən} (\textsc{seq}) (Converbal affix; Indirect inflection)
...
\textit{-tɨ} (\textsc{seq}) can directly follow the verbal root. Basically, it is used in affirmative as in (\ref{ex:8-102}a-b). In contrast, \textit{-nən} (\textsc{seq}) is always preceded by \textit{-an} (\textsc{neg}), i.e. always used in negative as in (\ref{ex:8-102}c-d).
...
\ea\label{ex:8-109}
  Verbal morphemes that can directly precede \textit{-i}/\textit{-∅} (\textsc{inf}) (Infinitival affix; Direct inflection)
...
The clause-final form may appear only in the case of (\ref{ex:8-110}c). The infinitives of (\ref{ex:8-110}a-c) cannot take their own subjects. In other words, in those cases, the subjects of infinitives always coincide with those of the main clauses. The stative verb \textit{ar-} can be followed by \textit{-i} (\textsc{inf}) in the conditions of (8-110 a, d) as in the examples (\ref{ex:8-111}c) and (\ref{ex:8-115}f). However, the copula verb cannot take the infinitival affix.

With regard to (\ref{ex:8-110}a), the infinitive can appear in the complement slot of the VP, where the lexical verb is always \textit{sɨr-} ‘do’ as in (\ref{ex:8-111}a-c). The infinitives take basic forms in this environment.
...
[give.a.feast-\textsc{ing}+want-\textsc{adj}  \textsc{stv}-\textsc{inf}=\textsc{top}]  [do-\textsc{act}-\textsc{ptcp}]=\textsc{advrs}\\
      \{[Complement]    [\textsc{lv}]\}\textsubscript{VP}\\
...
The above examples show that the infinitives fill the complement slots of the VPs composed of the light verb \textit{sɨr-} ‘do.’ These structures are called the light verb construction, and details will be disscussed in \sectref{sec:9.1.2.}
  ...
  With regard to (\ref{ex:8-110}b), the infinitive can become the core argument of the nominal predicate as in (\ref{ex:8-112}a-c) (see \sectref{sec:9.3} for more details on nominal predicate). The infinitives take basic forms in this environment.
...
It should be noted that the infinitive /kawai/ \textit{kawar-i} (change-\textsc{inf}) ‘changing’ in (\ref{ex:8-112}b) retains its own argument \textit{heisjei=kaci} (Heisei=\textsc{all}) ‘to Heisei.’ Similarly, the infinitive /hanasi/ \textit{hanas-i} (talk-\textsc{inf}) ‘talking’ in (\ref{ex:8-112}c) retains its own argument \textit{cˀju=nu} \textit{sima+cˀju=tu} (person=\textsc{gen} community+person=\textsc{com}) ‘with a person from another community.’
  ...
  With regard to (\ref{ex:8-110}c), the infinitive can be used in the predicate slot in the main clause. The infinitives take either basic forms or clause-final forms in this environment (see \sectref{sec:8.4.4.1} for more details). The infinitive in the predicate slot may be followed by a copula verb as in (\ref{ex:8-113}a-c). That is, it forms a nominal predicate phrase.
...
The nominative particle has two forms \textit{ga} and \textit{nu}. The former \textit{ga} (\textsc{nom}) is used when the preceding nominal belongs to the higher position in the animacy hierarchy, and the latter \textit{nu} (\textsc{nom}) is used when the preceding nominal belongs to the lower position in the animacy hierarchy (see \sectref{sec:6.4} for more details). Therefore, \textit{mizjuu} ‘ditch’ normally takes \textit{nu} (\textsc{nom}) as in (\ref{ex:8-114}b), since it indicates an inanimate referent, which is in the lowest position in the animacy hierarchy. However, if the predicate is a filled by an NP, i.e. a nominal predicate, the subject always takes \textit{ga} (\textsc{nom}) irrespective the animacy of the preceding nominal (see \sectref{sec:6.4.3.2} for more details). Thus, \textit{mizjuu} ‘ditch’ in (\ref{ex:8-114}a) takes the nominative particle \textit{ga} (not \textit{nu}). This means the infinitive \textit{ik-i} (go-\textsc{inf}) ‘going’ fills the predicate slot of the main clause, and the subject is \textit{mizjuu} ‘ditch.’ In other words, \textit{mizjuu} ‘ditch’ and \textit{ik-i} (go-\textsc{inf}) does not form a single (nominal) clause. Otherwise, the alleged nominal clause as a whole would fill the predicate of the main clause, where the subject of the \textit{ik-} ‘go’ has to take the nominative particle \textit{nu}, since the internal syntax of the alleged nominal clause does not require \textit{mizjuu} ‘ditch’ to take \textit{ga} (\textsc{nom}). Considering the above examples, we can conclude that the infinitive as the nominal predicate in the main clause (or complement clause) is a verbal form that can retain its arguments with the exception of the subject. The infinitive followed by \textit{n} (\textsc{dat1}), however, is not the case since it can retain the subject’s nominative \textit{nu} as in \textit{a-n} \textit{cˀju=nkja=nu} (\textsc{dist}-\textsc{adnz} person=\textsc{appr}=\textsc{nom}) in (\ref{ex:8-115}b) below.
  ...
  With regard to (\ref{ex:8-110}d), if the infinitive is followed by \textit{n} (\textsc{dat1}), it can indicate a certain temporal point as in (\ref{ex:8-115}a-f). The infinitives take basic forms in this environment.
...
\textsc{dist}-\textsc{ptcp}  person=\textsc{appr}=\textsc{nom}  principal+teacher   do-\textsc{prog}-\textsc{inf}=\textsc{dat1}=\textsc{nom}  Amuro=\textsc{loc1}  do-\textsc{inf}=\textsc{dat1}=\textsc{nom}  Amuro=\textsc{nom}  fire=\textsc{nom}  do-\textsc{pst}-\textsc{ptcp}  time=\textsc{infr}=\textsc{cfm3}\\
...
2.\textsc{hon}-\textsc{pl}=\textsc{nom}  do-\textsc{seq}  \textsc{hon}-\textsc{inf}=\textsc{dat1}=\textsc{top}  island=\textsc{gen}   fish=\textsc{infr}=\textsc{cfm3}\\
...
\textit{naa}  \textit{u-rɨ=ga}  \textit{jˀ-i=ga=joo}  \textit{hɨru}  \textit{kam-i}  \textit{gakkjuu}    \textit{kam-i}  \textit{zjagaimo}  \textit{kam-i}  \textit{hansɨ}  \textit{kam-i}  \textit{koosjaa}  \textit{kam-i}    \textit{unuu}  \textit{kam-i=cjɨ=nkja}  \textit{umuw-jur-n=ccjɨ=joo}\\
    \textsc{fil}  \textsc{mes}-\textsc{nlz}=\textsc{nom}  say-\textsc{inf}=\textsc{nom}=\textsc{cfm1}  garlic  eat-\textsc{inf}  shallot  eat-\textsc{inf}  potato  eat-\textsc{inf}  sweet.potato  eat-\textsc{inf}  yam  eat-\textsc{inf}  taro  eat-\textsc{inf}=\textsc{qt}=\textsc{appr}  think-\textsc{act}-\textsc{ptcp}=\textsc{qt}=\textsc{cfm1}\\
...
now=\textsc{abl}  book  read-\textsc{act}-\textsc{npst}=\textsc{plq}\\
...
\subsubsection{Affixes following infinitives}\label{sec:8.4.4.3}

There are affixes that always follow infinitives.

\ea \label{ex:8.119}
\ea Nominalizers \label{ex:4.27a}
\begin{xlisti} \ex  \textit{-zjaa} ‘place,’ \textit{-bəə} ‘role’
\ex \textit{-mai} (\textsc{obl}), \textit{-madəə} ‘fail to,’ \textit{-gjaa} (\textsc{purp})
\end{xlisti}
\ex Adjectivalizers\\\label{ex:4.27b}
    \textit{-cja} ‘want,’ \textit{-cjagɨ} ‘seem,’ \textit{-jass} ‘easy,’ \textit{-gussj} ‘difficult’
\z
\z  

The affixes in (\ref{ex:8.119}a) change the preceding verbal stems into nominal stems, and the affixes in (\ref{ex:8.119}b) change the preceding verbal stems into adjectival stems. Semantically, the affixes in (\ref{ex:8.119}a-i) have more concrete meaning than those in (\ref{ex:8.119}a-ii). In fact, the former can be an argument NP, but the latter cannot. The words composed of the affixes in (\ref{ex:8.119}a-ii) can fill the predicate slot, complement slot, or NP modifier slot.

I will present the first type of nominalizers, i.e. \textit{-zjaa} ‘place’ and \textit{-bəə} ‘role,’ in the following examples, in which the words (composed of the affixes) are argument NPs as in (\ref{ex:8.120}b) and (\ref{ex:8.121}c), and predicate NPs as in (\ref{ex:8.120}a) and (\ref{ex:8.121}a, b). Those words are underlined in the following examples.

\ea  \textit{-zjaa} ‘place’ \label{ex:8.120}
\ea\label{ex:8.120a}
%  \textsc{tm}:
\glll  umaga  asɨbizjaa  jatattujaa.\\
...
\ea  \textit{-bəə} ‘role’ \label{ex:8.121}
% \textsc{tm}:
\ea\label{ex:8.121a}
...
\glll     waakja  {\textbar}nenzjuu{\textbar}  mɨzɨkˀumbəə  jatattu.\\
      ...
      (The singular meaning of \textit{waakja} is discussed in \sectref{sec:5.1.1})  
...
The composition of these words are very similar to the V+N compound in (4-25 b) in \sectref{sec:4.2.3.1}, e.g. \textit{hing-i+madu} (escape-\textsc{inf}+time). However, \textit{-zjaa} ‘place’ and \textit{-bəə} ‘role’ are crucially different from \textit{madu} ‘time’ in that they cannot be analyzed as filling the head slot of an NP. As is shown in (\ref{ex:8.120}a-b), they cannot be modified by NP modifiers such as adnominal clauses.

\ea  Cannot be modified by adnominal clauses \label{ex:8.120}
\ea\label{ex:8.120a}
%  \textsc{tm}:
\glll   *kumoo  asɨbjun  zjaadoo.\\
       \textit{ku-ma=ja}  \textit{\Highlight{asɨb-jur-n}}  \textit{\Highlight{-zjaa}=doo}\\
      \textsc{prox}-place=\textsc{top}  play-\textsc{act}-\textsc{ptcp}  place=\textsc{ass}\\
      \glt       (Intended meaning) ‘Here is the place to play.’ [El: 130816]

\ex\label{ex:8.120b} % \textsc{tm}:
\glll  *arəə  ucjun  bəədoo.\\
       \textit{a-rɨ=ja}  \textit{\Highlight{ut-jur-n}}  \textit{\Highlight{-bəə}=doo}\\
       \textsc{dist}-\textsc{nlz}=\textsc{top}  hit-\textsc{act}-\textsc{ptcp}  role=\textsc{ass}\\
...
By contrast, the noun \textit{madu} ‘time’ can be modified by an adnominal clause just as in the case of ordinary nouns as in (\ref{ex:8.123a}). Additionally, \textit{madu} ‘time’ can be used without any NP modifier as in (\ref{ex:8.123b}). The particles \textit{-zjaa} ‘place’ and \textit{-bəə} ‘role’ cannot be used in either case.

\ea \label{ex:8.123}
\ea Can be modified by an adnominal clause\\\label{ex:8.123a}
% \textsc{tm}:
\glll  asɨbjun  madunkjoo  nən.\\
    \textit{asɨb-jur-n}  \textit{\Highlight{madu}=nkja=ja}  \textit{nə-an}\\
    \{[play-\textsc{act}-\textsc{ptcp}]\textsubscript{Adnominal clause} time\}\textsubscript{NP}=\textsc{appr}=\textsc{top}  exist-\textsc{neg}\\
    \glt     ‘There is no time to play.’ [El: 130816]

\ex Can be used without any NP modifier\\\label{ex:8.123b}
\glll  uroo  madoo  nənna?\\
    \textit{ura=ja}  \textit{\Highlight{madu}=ja}  \textit{nə-an=na}\\
    2.N\textsc{hon}.\textsc{sg}=\textsc{top}\textsubscript{} \{time\}\textsubscript{NP}=\textsc{top}  exist-\textsc{neg}=\textsc{plq}\\
    \glt     ‘Don’t you have the time?’ [El: 130816]
\z
\z

The comparison between \textit{-zjaa} ‘place’ and \textit{-bəə} ‘role’ on one hand, \textit{madu} ‘time’ on the other indicates that the former morphemes are affixes which cannot head an NP by itself. Hence, they are analyzed as nominalizer affixes.

The second type of nominalizers involve \textit{-mai} (\textsc{obl}), \textit{-madəə} ‘fail to,’ and \textit{-gjaa} (\textsc{purp}). They are similar to \textit{-zjaa} ‘place’ and \textit{-bəə} ‘role’ in that they are always preceded by verbal infinitives and cannot head an NP. In \REF{ex:4.31}, \textit{-mai} (OBL) serves as the nominal predicate.
...
% \textsc{tm}:
  \gllll təəhunu  kjuncjuuba,  amɨn   huimaidoojaa.\\
    ...
    typhoon=\textsc{nom}  come-\textsc{act}-\textsc{ptcp}=\textsc{qt}+say-\textsc{csl}  [rain]=also  [fall-\textsc{inf}-\textsc{obl}]=\textsc{ass}=\textsc{sol}\\
        [Subject]                                         [Nominal predicate]\\
  \glt ‘Since (they said ) that the typhoon will come, it must rain [lit. the rain must fall].’  [El: 120929]
\z

\noindent
This epistemic use of \textit{-mai} (\textsc{obl}) is only attested in elicitation. Just for reference, I want to mention that the ancestral form of \textit{-mai} could be *mari  (Joel Bradshow, 2018 p.c.), since (1) the ancestral /ai/ sequence has become /əə/ in the modern Yuwan, e.g. */kaina/ > /kəənja/ ‘arm’ and */aida/ > /əəda/ ‘space,’ and (2) the modern /ai/ sequence has been derived from */ari/, e.g. */putari/ > /tˀai/ ‘two people’ and */hari/ > /hai/ ‘needle.’

In \REF{ex:4.33}, \textit{-madəə} depicts that the action denoted by the stem failed to complete. Syntactically, the compound fills the predicate slot as in \REF{ex:4.33a} or fills the complement slot of the light verb construction (\textsc{lvc}) as in \REF{ex:4.33b}.
...
\ex \textit{-madəə} ‘fail to’ in the complement slot of \textsc{lvc} \label{ex:4.33b}\\
...
The final example is \textit{-gjaa} (\textsc{purp}), which means that the subject has a purpose to do the action indicated by the verbal stem. Syntactically, it fills the predicate as in \REF{ex:4.34a} or is followed by the genitive case as in \REF{ex:4.34b}. Additionally, it can fill the complement slot of the deictic motion verbs \textit{ik-} ‘go’ and \textit{k-} ‘come’ as in (\ref{ex:4.34}c-d).
...
\gllll 
    {\textbar}sjoogacu{\textbar}ja,  naa,  jˀuuboo,  namanu  {\textbar}nentoo{\textbar}   jˀiigjaa  jappa.\\
      ...
      New.Year’s.day=\textsc{top}  \textsc{fil}  say-\textsc{cnd}  now=\textsc{gen}  beginning.of.a.year                           [say-\textsc{inf}-\textsc{purp}  say-\textsc{csl}]\\
        {} {} {} {} {} [Nominal\text{ }predicate]\\
      \glt ‘The New Year’s day. (Briefly) speaking in modern (words, it is the day when the relatives gather just) in order to say New Year's greetings.’ [Co: 111113\_01.txt]
...
\gllll    jˀiigjaanu  cɨmuisjɨ  acɨmajunwakejo.\\
      ...
      [say-\textsc{inf}-\textsc{purp}]=\textsc{gen}  intention=\textsc{inst}  gather-\textsc{act}-\textsc{ptcp}=\textsc{cfp}=\textsc{cfm}1\\
      [NP]=\textsc{gen}    \\
      \glt       ‘(The relatives) gather (as if) they intended to say (the New Year's greetings).’ [Co: 111113\_01.txt]

\ex \textit{-gjaa} (\textsc{purp}) in the complement slot of \textit{ik-} ‘go’\\\label{ex:4.34c}
\gllll   usi  tuigjaa  izjattoo,\\
      \textit{usi}  \textit{\Highlight{tur-i-gjaa}}  \textit{ik-tar-too}\\
      cow  [take-\textsc{inf}-\textsc{purp}]  [go-\textsc{pst}-\textsc{csl}]\\
        {} [Complement]  [Lexical\text{ }verb]\\
...
It should be mentioned that the preceding verbal stems followed by some of the nominalizers in this section can retain their original argument structure (or “internal syntax” in \citealt{Haspelmath1996}: 52) as in (\ref{ex:4.35}b-c). The same is true of the compound composed of \textit{madu} ‘time’ (\ref{ex:4.35}d).

\ea  \label{ex:4.35}
\ea Original argument structure \label{ex:4.35a}\\
% \textsc{tm}:
\gllll  wanna  urɨba  kakjuttoo.\\
    \textit{wan=ja}  \textit{u-rɨ=ba}  \textit{kak-jur=doo}\\
    1\textsc{sg}=\textsc{top}  \textsc{mes}-\textsc{nlz}=\textsc{acc}  write-\textsc{act}=\textsc{ass}\\
      {} Object  \\
    \glt     ‘I will write it.’ [El: 130816]

\ex \textit{-bəə} ‘role’\\
\gllll   wanna  urɨba  kakibəə  zjajaa.\\
...
\ex \textit{madu} ‘time’ (Example of compound)\\
\gllll  wanna  urɨnkjoo  kakimadoo  nəndoo.\\
...
The example in (\ref{ex:4.35}a) shows the original argument structure of \textit{kak-} ‘write,’ whose object \textit{u-rɨ} ‘that’ is marked by \textit{ba} (\textsc{acc}). The examples in (\ref{ex:4.35}b-c) show that the nominalized (or compounded) \textit{kak-} ‘write’ still retains its object, although I was unable to have the speaker say an example where the object of \textit{kak-i+madu} (write-\textsc{inf}+time) was marked by \textit{ba} (ACC). Furthermore, \textit{-zjaa} ‘place’ cannot retain its original argument structure, e.g. */kumoo mɨzɨba numzjaadoo/ \textit{ku-ma=ja} \textit{mɨzɨ=ba} \textit{num-∅-zjaa=doo} (\textsc{prox}-place=\textsc{top} water=ACC drink-INF-place=\textsc{ass}) [Intended meaning] ‘Here is the place to drink water.’

It is noteworthy that Yuwan has two types of nominalizers. First type, i.e. the nominalizers discussed in this section, always need the preceding verbal stems to be infinitives. In other words, these nominalizers cannot follow any verbal root directly; i.e. *\textit{hik-bəə} (play-role) is ungrammatical. In contrast, the other type, i.e. \textit{-jaa} ‘person,’ can directly attach to verbal roots, e.g. /hikjaa/ \textit{hik-jaa} (play-person) ‘player’ (see \sectref{sec:7.6}). 

Finally, I will present adjectivalizers in Yuwan: \textit{-cja} ‘want,’ \textit{-cjagɨ} ‘seem,’ \textit{-jass} ‘easy,’ and \textit{-gussj} ‘difficult.’ In principle, these adjectivalizers always follow the verbal infinitives. The example of \textit{-cja} ‘want’ is shown below, and other examples are shown in \sectref{sec:4.3.8.2}.
...
The adjectival stems can retain the original argument structure of the internal verbal stems. For example, \textit{sima+kutuba} ‘the language of the community’ is the argument (i.e. object) of \textit{naraw-} ‘learn’ in \REF{ex:4.36}.
...
\\
    male=\textsc{gen}  child=\textsc{top}  pear  \textsc{mes}-\textsc{adnz}  scatter-\textsc{pst}-\textsc{csl}  \textsc{fil}   continuously  \textsc{prox}-\textsc{advz}  do-\textsc{seq}  do-\textsc{act}-\textsc{seq}  have.a.pain-\textsc{act}-\textsc{ptcp}=\textsc{qt}  say-\textsc{prog}-\textsc{seq}\\
...
\textit{akira=ga}  \textit{hon}  \textit{koow-jur-i}\\
    Akira=\textsc{nom}  book  buy-\textsc{act}-\textsc{npst}\\
...
1\textsc{sg}=\textsc{nom}  \textsc{dist}-\textsc{advz}  go-\textsc{act}-\textsc{pol}=\textsc{qt}\\
...
\subsubsection{\textit{-jur} (\textsc{act})}

\textit{-jur} (\textsc{act}) has multiple functions and it’s prototypical function is difficult to determine. In principle, it has the characteristics as in \REF{ex:8-131}; see also \REF{ex:8-1} and \REF{ex:8-2} in \sectref{sec:8.1.}

\ea\label{ex:8-131}
  Morphologically, \textit{-jur} (\textsc{act})

\ea Cannot co-occur with \textit{-arɨr} (\textsc{pass})\footnote{From the description in \sectref{bkm:Ref303739828}, one may think of the combination of \textit{-arɨr-tuk-jur} (\textsc{pass}-\textsc{prpr}-\textsc{act}). However, the combination of \textit{-arɨr} (\textsc{pass}) and \textit{-tuk} (PRPR) is rare (see \sectref{bkm:Ref366851018}), and the combination more than two derivational affixes is also rare (see \sectref{bkm:Ref303739828}). Thus, we may postulate that \textit{-jur} (act) cannot co-occur with (or at least rarely co-occurs with) \textit{-arɨr} (\textsc{pass}).} or \textit{-arɨr} (\textsc{cap});
...
With regard to (\ref{ex:8-131}a), \textit{-jur} (\textsc{act}) necessarily indicates the active voice. In Yuwan, there are three affixes that have the valency-changing function: \textit{-as} (\textsc{caus}), \textit{-arɨr} (\textsc{pass}), and \textit{-arɨr} (\textsc{cap}). Thus, its incapability of co-occurence with \textit{-arɨr} (\textsc{pass}) and \textit{-arɨr} (CAP) greatly reduces the possibility of the change of valency.

With regard to (\ref{ex:8-131}b), \textit{-jur} (\textsc{act}) cannot co-occur with the negative affixes, i.e. \textit{-an} (\textsc{neg}) as in \REF{ex:8-1} in \sectref{sec:8.1} or \textit{-azɨi} (\textsc{neg}.\textsc{plq}) as in \REF{ex:8-67} in \sectref{sec:8.4.1.4.} Yuwan does not have another method to express the negative polarity. Thus, the existence of \textit{-jur} (act) necessarily indicates the affirmative polarity.

  With regard to (\ref{ex:8-131}c), \textit{-jur} (\textsc{act}) necessarily indicates non-progressive aspect. In Yuwan, there are three affixes (except for \textit{-jur}) that have aspectual meaning: \textit{-tuk} (\textsc{prpr}), \textit{-tur} (\textsc{prog}), and \textit{-təər} (\textsc{rsl}). Among them, \textit{-tuk} (PRPR) and \textit{-təər} (RSL) can co-occur with \textit{-jur} (act). The combination of \textit{-jur} (act) and \textit{-tuk} (PRPR) will be discussed in \sectref{sec:8.5.1.7.} The combination of \textit{-jur} (act) and \textit{-təər} (RSL) requires a special attention and it will be discussed in later in this section.

With regard to (\ref{ex:8-131}d), \textit{-jur} (\textsc{act}) necessarily indicates the non-polite style, although it does not necessarily mean the rudeness in a general sense, since \textit{-jur} (act) can co-occur with the honorific expression (see \sectref{sec:8.3.1} for more details).

Additionally, \textit{-jur} (\textsc{act}) belongs to the indirect inflections, which are required by some inflectional affixes such as \textit{-i} (\textsc{npst}) or \textit{-mɨ} (\textsc{plq}), since those inflectional affixes cannot directly follow the verbal root (see (\ref{ex:8-3}b) in \sectref{sec:8.1} for more details).

  Considering the above facts, i.e. the active voice, the affirmative polarity, the non-progressive aspect, the non-politeness, and the necessity to some inflections, I propose that \textit{-jur} has some “unmarked” characteristics and abbreviate them as “\textsc{act}” in this grammar. I will show the examples of \textit{-jur} (act) below.
...
\z
  ...
  In addition, \textit{-jur} (\textsc{act}) can express habitual aspect if it precedes \textit{-tar} (\textsc{pst}), \textit{-tɨ} (\textsc{seq}), or \textit{-təər} (\textsc{rsl}) as shown in (\ref{ex:8-133}a-g).

\ea\label{ex:8-133}
  \textit{-jur} (\textsc{act}) expressing habitual aspect
...
2.\textsc{hon}.\textsc{sg}-\textsc{adnz}  house=\textsc{all}=\textsc{top}  always  go-\textsc{act}-\textsc{pst}-\textsc{ptcp}=\textsc{advrs}\\
...
young-\textsc{adj}+\textsc{stv}-\textsc{inf}=\textsc{dat1}=\textsc{top}  Minoe-\textsc{pl}=\textsc{appr}=\textsc{com} play-\textsc{act}-\textsc{pst}-\textsc{ptcp}=\textsc{qt}\\
...
necessarily  \textsc{dist}-place=\textsc{loc1}  sing-\textsc{caus}-\textsc{act}-\textsc{pst}=\textsc{ass}\\
...
\textsc{mes}-\textsc{advz}  do-\textsc{pst}-\textsc{ptcp}  thing=\textsc{appr}  know-\textsc{prog}-\textsc{ptcp}  person=\textsc{nom}  exist-\textsc{neg}-\textsc{csl}=\textsc{qt}  say-\textsc{prog}-\textsc{seq}=\textsc{foc}  always  say-\textsc{act}-\textsc{pst}-\textsc{ptcp}=\textsc{advrs}  2.\textsc{nhon}.\textsc{sg}  ask-\textsc{cnd}  good-\textsc{adj}  \textsc{stv}-\textsc{pst}\\
...
when=any  1\textsc{pl}=\textsc{top}  go-\textsc{act}-\textsc{seq}  \textsc{mes}-\textsc{nls}  do-act-\textsc{pst}-\textsc{pol}\\
...
\textsc{prox}-\textsc{advz}  do-\textsc{pst}-\textsc{ptcp}  \textsc{mes}-\textsc{nlz}=\textsc{appr}=\textsc{top}  when=\textsc{abl}   wear-\textsc{act}-\textsc{seq}\\
...
\textsc{mes}-\textsc{nlz}=also  do-\textsc{seq}  what-\textsc{indfz}  do-\textsc{seq}  \textsc{hon}-\textsc{act}-\textsc{rsl}-\textsc{ptcp}=\textsc{cfp}=\textsc{cfm1}\\
\glt ‘(The person) did it too, and used to do (everything, and we can still see the results ).’ [Co: 120415\_01.txt]
\z
\z

The above examples show that the combinations of \textit{-jur} (\textsc{act}) with \textit{-tar} (\textsc{pst}), \textit{-tɨ} (\textsc{seq}), or \textit{-təər} (\textsc{rsl}) can express habitual meaning. The habitual meaning of the clauses are also expressed by the co-occuring temporal words, i.e. \textit{nenzjuu} ‘always’ as in (\ref{ex:8-133}a) and /ɨcɨn/ \textit{ɨcɨɨ=n} (when=any) ‘always’ as in (\ref{ex:8-133}e).

  In fact, there are a few examples where the combination of \textit{-jur-tar} (\textsc{act}-\textsc{pst}) does not express habitual meaning as in (\ref{ex:8-134}a-b).
...
recently=\textsc{foc}  \textsc{prox}-\textsc{nlz}=\textsc{top}  have-\textsc{seq}  come-\textsc{act}-\textsc{pst}\\
...
how-\textsc{advz}  do-\textsc{seq}  go-\textsc{act}-\textsc{pst}=\textsc{dub}  America=\textsc{all}=\textsc{appr}=\textsc{top}  America=\textsc{all}=\textsc{top}  how-\textsc{advz}  do-\textsc{seq}  cross.over-\textsc{act}-\textsc{pst}=\textsc{dub}\\
...
In (\ref{ex:8-134}a), the event expressed by the clause (i.e. Satsue’s child’s bringing the picture) took place only once. Thus, \textit{-jur} (\textsc{act}) in this example cannot express habitual aspect. Similarly, the event in (\ref{ex:8-134}b-d) (i.e. the Uncle America’s crossing over to the US) took place only once. \textsc{tm}’s utterance in (\ref{ex:8-134}b) is almost the same with that in (\ref{ex:8-134}d), where \textit{-jur-tar} (act-\textsc{pst}) in (\ref{ex:8-134}b) is replaced by \textit{-tar} (\textsc{pst}). The details of the function of \textit{-jur} (act) in (\ref{ex:8-134}a-b) is not very clear for the present author for now, and a finer investigation is required in the future.
...
what  do-\textsc{act}-\textsc{seq}=\textsc{foc}  play-\textsc{prog}-\textsc{pst}=\textsc{dub}\\
...
where=\textsc{all}=\textsc{dub}  make-\textsc{caus}-\textsc{seq}  come-\textsc{inf}  \textsc{cop}-\textsc{pst}-\textsc{infr}=\textsc{sol}   Yuwan+person=\textsc{top}  make-\textsc{act}-\textsc{ptcp}  person=\textsc{top}  exist-\textsc{neg}-\textsc{rsl}-\textsc{csl}\\
...
that.time-around=\textsc{abl}  tone.deaf=\textsc{qt}  think-\textsc{neg}-\textsc{seq}  well  do.one’s.best-\textsc{seq}  learn-\textsc{cnd}    good.at  become-\textsc{act}-\textsc{rsl}-\textsc{ptcp}=\textsc{advrs}=\textsc{ass}=\textsc{sol}\\
...
study  become-\textsc{act}-\textsc{rsl}-\textsc{ptcp}=\textsc{advrs}=\textsc{ass}\\
\glt ‘(If you had asked him, it) must have become good study (for you), but (it did not become so).’ [Co: 111113\_02.txt]
\z
\z

All of the above examples have the conditional adverbial clauses (i.e. protasis), overtly in (\ref{ex:8-141}a-b) and covertly in (\ref{ex:8-141}c), and these adverbial clauses express counter-factual situations. Thus, the superordinate clauses that express their conclusions (i.e. apodosis) also express counter-factual situations, where \textit{-təər} (\textsc{rsl}) is used. The use of \textit{-təər} (RSL) as in (\ref{ex:8-141}b) provides a clear contrast to \textit{-tar} (\textsc{pst}) as in (\ref{ex:8-133}d) in \sectref{sec:8.5.1.4.} In (\ref{ex:8-141}b), \textit{nar-jur-təər-n=mun} (become-\textsc{act}-RSL-\textsc{ptcp}=\textsc{advrs}) ‘would have become (good at singing), but ...’ expresses a counter-factural situation. In contrast, in (\ref{ex:8-133}d), \textit{jˀ-jur-tar-n=mun} (say-act-\textsc{pst}-\textsc{ptcp}=ADVRS) ‘used to say (a phrase), but ...’ expresses the real fact.
...
other+moment=\textsc{foc}  do-\textsc{inf}+\textsc{cap}-\textsc{act}-\textsc{npst}=\textsc{cfm1}\\
...
\{[write-\textsc{seq}]  [\textsc{hon}-\textsc{inf}]\}+\textsc{cap}-\textsc{act}-\textsc{npst}=\textsc{plq}\\
