...
\noindent The personal pronominals cannot become adverbs. There are no cross-over categories that become verbs, adjectives, particles, or interjections. The difference between cross-over categories and verbs will be discussed in the \sectref{sec:8.4.5}.
...
\noindent In \REF{ex:5:3a} \textsc{tm} uses \textit{wattəə} (1\textsc{du}) ‘the two of us’ to include the hearer US, and in \REF{ex:5:3b} she uses the same form to exclude the hearer \textsc{ms}.
...
\Highlight{\textit{wan=tu}}  \Highlight{\textit{urattəə}}  \textit{ik-tɨ} \textit{k-an-boo}\\
...
Yuwan has a numeral word that indicates two people (see \sectref{sec:7.4}), i.e. \textit{tˀai} ‘two people.’ This numeral word can be used with both of the dual pronouns and the plural pronouns.
...
\noindent In \REF{ex:5:7a}, \textsc{tm} uses \textit{waakja} (1\textsc{pl}) ‘we (all)’ including the hearer, and in \REF{ex:5:7b} she uses the same form excluding the hearer.
...
\glt \hspaceThis{\textsc{my}:} ‘Neither do I.’ [Co: 110328\_00.txt]
\z
...
\matrix (persons) [ampersand replacement=\&, row sep=5mm]
  {
    ...
    \node {Dual};     \& \node[bob,minimum size=12pt,hair=lsNightGreen,skin=lsNightGreen,shirt=lsNightGreen,undershirt=lsNightGreen] (dual1) {}; \& \node[bob,minimum size=12pt,hair=lsRichGreen,skin=lsRichGreen,shirt=lsRichGreen,undershirt=lsRichGreen] (dual2) {};\\
    \node {Plural};   \& \node[bob,minimum size=12pt,hair=lsNightGreen,skin=lsNightGreen,shirt=lsNightGreen,undershirt=lsNightGreen] (plural1) {}; \& \node[bob,minimum size=12pt,hair=lsRichGreen,skin=lsRichGreen,shirt=lsRichGreen,undershirt=lsRichGreen] (plural2) {}; \& \node{...}; \& \node[bob,minimum size=12pt,hair=lsSoftGreen,skin=lsSoftGreen,shirt=lsSoftGreen,undershirt=lsSoftGreen] {}; \& \node (plural3) {...}; \&[1mm]
        \node{or};     \&[1mm] \node[bob,minimum size=12pt,hair=lsNightGreen,skin=lsNightGreen,shirt=lsNightGreen,undershirt=lsNightGreen] (plural4) {}; \&
                \node[bob,minimum size=12pt,hair=lsSoftGreen,skin=lsSoftGreen,shirt=lsSoftGreen,undershirt=lsSoftGreen] {}; \& \node (plural5) {...};\\
  };
...
\begin{flushleft}
\tikz[baseline] \node (node) [bob,minimum size=12pt,hair=lsNightGreen,skin=lsNightGreen,shirt=lsNightGreen,undershirt=lsNightGreen] {}; : speaker (specific)
...
\hfill\tikz[baseline] \node (node) [bob,minimum size=12pt,hair=lsSoftGreen,skin=lsSoftGreen,shirt=lsSoftGreen,undershirt=lsSoftGreen] {}; : non-speaker (unspecific)\\
\hfill\tikz[baseline] \draw (0,0) rectangle (1,.5); : group (definite) 
\hfill\tikz[baseline] \draw [dashed] (0,0) rectangle (1,.5); : group (indefinite)
\hfill\hbox{}
\end{flushleft}
\end{figure}

This figure shows that the plural form \textit{waakja} (1\textsc{pl}) in \REF{ex:5:8} indicates an indefinite group as in the figure surrounded by the dashed square in \figref{fig:key:7}. In that group, the specific referent is only the speaker, and the group itself is difficult to be identified by the hearer (i.e. indefinite). The details of this phenomenon is discussed in \citet{Niinaga2020b}.
This kind of plural meaning is also expressed in the second-person pronominals discussed in the next section (see also the discussion in \sectref{sec:6.4.1.1}).
...
\glt ‘About the day you said (about the visit from the present author), ...’ [Co: 110328\_00.txt]
 ...
 \ex \label{ex:5:9b} \textit{ura} (2.N\textsc{hon}.\textsc{sg})
...
\noindent In \REF{ex:5:9a}, \textsc{tm} is speaking to US, who is older than TM, so TM has to use the honorific form of the second-person pronoun. On the other hand, in \REF{ex:5:9b}, TM is speaking to \textsc{ms}, who is younger than TM, so TM uses the non-honorific form of the second-person pronoun.
...
\z
  ...
  As mentioned in \sectref{sec:5.1.1}, the plural affix for personal pronominals, i.e. \textit{{}-kja} (\textsc{pl}), can indicate not only a definite group, but also an indefinite group. These meanings are illustrated below.
...
\begin{flushleft}
\tikz[baseline] \node (node) [bob,minimum size=12pt,hair=lsNightGreen,skin=lsNightGreen,shirt=lsNightGreen,undershirt=lsNightGreen] {}; : hearer (specific)
...
\hfill\tikz[baseline] \node (node) [bob,minimum size=12pt,hair=lsSoftGreen,skin=lsSoftGreen,shirt=lsSoftGreen,undershirt=lsSoftGreen] {}; : non-hearer (unspecific)\\
\hfill\tikz[baseline] \draw (0,0) rectangle (1,.5); : group (definite) 
\hfill\tikz[baseline] \draw [dashed] (0,0) rectangle (1,.5); : group (indefinite)
\hfill\hbox{}
\end{flushleft}
...
\noindent Here, \textit{naakja} (2.\textsc{hon}.\textsc{pl}) in (5-11 a) indicates an indefinite group as in the figure surrounded by the dashed square in \figref{fig:key:8}. In that group, the specific referent is only the hearer, and the other members are thought to be unspecific people who would not carry the miscanthus. Likewise, \textit{urakja} (2.\textsc{nhon}.PL) in (5-11 b) indicates an indefinite group as in the figure surrounded by the dashed square in \figref{fig:key:8}. In that group, the specific referent is only the hearer, and the other members are thought to be unspecific people who went to the main island of Japan (to find a job).

\subsection{Third person} \label{sec:5.1.3}
...
\noindent Strictly speaking, the number affixes in \REF{ex:5:14} also function as nominalizers. In the above morphemes, \textit{waa-} (1) and \textit{naa-} (2.\textsc{hon}) must conform to the phonological rule discussed in \sectref{sec:2.4.5}, which deletes a vowel in a vowel sequence. The zero morpheme \textit{{}-∅} is ignored in the rule.
...
\noindent Adopting the above analysis, I propose the following paradigm. (The following paradigm shows the underlying froms. About the surface form paradigm, see Tables \ref{tab:key:31}--\ref{tab:key:32} in \sectref{sec:5.1}.)
...
In \REF{ex:5:21}, \textit{a-n} (\textsc{dist}-\textsc{adnz}) ‘that (one)’ fills the modifier slot of an NP whose head is \textit{cˀju} ‘person.’ These types of demonstrative adnominals can be directly followed by locative cases (except for \textit{zjɨ}).
...
\noindent The above phenomena may be regarded as headless NPs. The same phenomenon occurs in the case of the interrogative adnominal \textit{dɨ-n} (which-\textsc{adnz}) ‘which (one)’ (see (5-40 a) in \sectref{sec:5.3.2}). Semantically, these forms express location, whose meaning is similar to that of \textit{{}-ma} ‘place.’ That is, the meaning of /kunnən/ \textit{ku-n=nən} (\textsc{prox}-\textsc{adnz}=\textsc{loc}1) ‘here’ (or /kunnan/ \textit{ku-n=nan} (\textsc{prox}-\textsc{adnz}=\textsc{loc}1) ‘here’) is almost the same as that of \textit{ku-ma=nan} (\textsc{prox}-place=\textsc{loc}1) ‘here’ (see also \sectref{sec:6.3.2.6}).
...
Interestingly, the combination composed of the demonstrative adverbs and the light verb \textit{sɨr-} ‘do’ can also redundantly modify another \textit{sɨr-} ‘do’ as in \REF{ex:5:29}. In this example the second light verb \textit{sɨr-an-boo} (do-\textsc{neg-cnd}) expresses a physical action (i.e. to change the angle of a picture), but the first light verb \textit{sjɨ} (do.\textsc{seq}) does not. Actually, the light verbs immediately following demonstrative adverbs \textit{kan/gan/agan} in \REF{ex:5:27} and \REF{ex:5:28} also show the same semantic abstractness with that in \REF{ex:5:29}.
...
\textit{\Highlight{ka-n}}  \textit{\Highlight{sɨr-tɨ}}  \textit{sɨr-an-boo}\\
...
In these examples, \textit{ka} (\textsc{dub}) and \textit{gajaaroo} (\textsc{dub}) function as the marker of indirect questions, which will be discussed in \sectref{sec:10.4.2} and \sectref{sec:10.4.3.}

\subsection{Indefinite use}

An interrogative word can function as an indefinite word when it is followed by certain particles, namely \textit{ka} (\textsc{dub}), \textit{gajaaroo} (\textsc{dub}), and \textit{n} ‘any.’ There are other words that express indefinite meaning, i.e. “indefinite pronouns,” which will be shown in \sectref{sec:7.5.}
...
\ea \label{ex:5:39}   Intrrogative nominals + \textit{ka} (\textsc{dub})
