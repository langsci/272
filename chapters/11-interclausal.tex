...
\ea\label{ex:11-6}  \textit{{}-tɨ} (SEQ) expressing the past tense as the insubordination
  ...
  \ea  TM:  naakjoo  injasainnja  dantɨ
      ...
      2.HON.PL=TOP  small-ADJ+STV-INF=DAT1=TOP  where=LOC2
...
\ea\label{ex:11-9}  \textit{kurɨr-} (BEN) \textit{+-ba} (CSL)
  ...
  \ex  TM:  umoojaganaa,  abɨtɨ  tabooppajoo.
...
The argumentation for (11-14) is shown in the following sections. First, I will present examples of the focus construction of \textit{du} (FOC) in \sectref{sec:11.3.1.} Then, I will present examples of the focus construction of \textit{ga} (FOC) in \sectref{sec:11.3.2.}
...
\ea\label{ex:11-17}  \ea \textit{du} (FOC) co-occuring with \textit{{}-i} (NPST)
  ...
  TM:  kunugurudu  kurəə  mucjɨ\footnote{\textit{mut-tɨ} (have-SEQ) usually becomes /muccjɨ/ according to the rule in \sectref{sec:key:8.3.1.2.} However, it becomes /mucjɨ/ in this example.}  kjuuta.
...
In (11-18 a-d), \textit{{}-u} (PFC) co-occurs with \textit{ga} (FOC). However, the existense of \textit{ga} (FOC) does not induce that of \textit{{}-u} (PFC). For example, \textit{ga} (FOC) in the (alleged) interrogative clause can appear without \textit{-u} (PFC) if it is followed by \textit{kai} (DUB) as in (11-8) in \sectref{sec:11.2.1.} Moreover, \textit{ga} (FOC) can be used in the non-interrogative clauses, where \textit{ga} (FOC) does not take \textit{{}-u} (FOC) as in (11-19) (see \sectref{sec:key:10.1.2.2} for more details).
